...
config.optionDefinitions = {
  ["main"] = {
    ...
    info = function()
      return ReadText(1001, 2655) ..
          ReadText(1001, 120) ..
          " " ..
          GetVersionString() ..
          "\n" ..
          ffi.string(C.GetBuildVersionSuffix()) ..
          (C.IsGameModified() and (ColorText["text_warning"] .. "(" .. ReadText(1001, 8901) .. ")") or "")
    end,
    [1] = {
      id = "onlineseason",
      ...
      wordwrap = true,
      display = function()
        return C.IsVentureSeasonSupported() and (not C.IsTimelinesScenario()) and
            (ffi.string(C.GetGameStartName()) ~= "x4ep1_gamestart_hub")
      end,
    },
    [2] = {
      id = "continue",
    ...
    [3] = {
      id = "load",
      name = function()
        return (menu.autoReloadSave or C.IsSaveListLoadingComplete()) and ReadText(1001, 2604) or
            ReadText(1001, 7203)
      end,
  ...
  ["timelines"] = {
    ...
    [1] = {
      id = "timelines_start",
      name = function()
        return (ffi.string(C.GetUserData("timelines_scenarios_finished")) ~= "") and
            ReadText(1001, 12620) or ReadText(1001, 12619)
      end,
      callback = function() return menu.callbackTimelines() end,
      selectable = function()
        return (not C.HasExtension("ego_dlc_timelines", false)) or
            C.IsExtensionEnabled("ego_dlc_timelines", false)
      end,
      mouseOverText = function()
        return (ffi.string(C.GetUserData("timelines_scenarios_finished")) ~= "") and
            ReadText(1026, 2698) or ReadText(1026, 2697)
      end,
    },
    [2] = {
      id = "timelines_reset",
      ...
      selectable = function()
        return C.IsExtensionEnabled("ego_dlc_timelines", false) and
            (ffi.string(C.GetUserData("timelines_scenarios_finished")) ~= "")
      end,
  ...
  ["settings"] = {
    ...
    [1] = {
      ...
      mouseOverText = function()
        return C.IsOnlineEnabled() and (ReadText(1026, 4806) .. "\n\n" .. ReadText(1026, 4807)) or
            ReadText(1001, 11592)
      end,
    },
    [2] = {
  ...
  ["display"] = {
    ...
    [4] = {
      ...
      mouseOverText = function()
        return (not C.GetDLSSOption(false)) and ReadText(1026, 4822) or
            (ColorText["text_error"] .. ReadText(1026, 4813))
      end,
      valuetype = "dropdown",
  ...
  ["input"] = {
    ...
    [12] = {
      id = "header",
      name = function()
        return (C.IsVROculusTouchActive() or C.IsVRViveControllerActive()) and ReadText(1001, 7229) or
            ReadText(1001, 2656)
      end,
    },
    [13] = {
    ...
    [44] = {
      ...
      display = function()
        return C.IsActiveHeadTrackerAvailable() and
            C.IsActiveHeadTrackerSettingSupported("filterstrength")
      end,
    },
    [45] = {
      ...
      display = function()
        return C.IsActiveHeadTrackerAvailable() and
            C.IsActiveHeadTrackerSettingSupported("anglefactor")
      end,
    },
    [46] = {
      ...
      display = function()
        return C.IsActiveHeadTrackerAvailable() and
            C.IsActiveHeadTrackerSettingSupported("deadzoneangle")
      end,
    },
    [47] = {
      ...
      display = function()
        return C.IsActiveHeadTrackerAvailable() and
            C.IsActiveHeadTrackerSettingSupported("positionfactor")
      end,
    },
    [48] = {
      ...
      display = function()
        return C.IsActiveHeadTrackerAvailable() and
            C.IsActiveHeadTrackerSettingSupported("deadzoneposition")
      end,
    },
    [49] = {
      ...
      display = function()
        return C.IsActiveHeadTrackerAvailable() and
            C.IsActiveHeadTrackerSettingSupported("gazefilterstrength")
      end,
    },
    [50] = {
      ...
      display = function()
        return C.IsActiveHeadTrackerAvailable() and
            C.IsActiveHeadTrackerSettingSupported("gazeanglefactor")
      end,
    },
    [51] = {
      ...
      display = function()
        return C.IsActiveHeadTrackerAvailable() and
            C.IsActiveHeadTrackerSettingSupported("gazedeadzone")
      end,
    },
    [52] = {
      id = "line",
  ...
  ["mouse_sensitivity"] = {
    ...
    [3] = {
      ...
      callback = function(value)
        return menu.callbackInputSensitivity(30, "sensitivity_direct_mouse_steering_pitch",
          value)
      end,
    },
    [4] = {
...
function menu.onGfxDone()
  ...
  if menu.userQuestion and menu.userQuestion.timer then
    menu.userQuestion.timer = getElapsedTime() +
        15.9 -- to avoid the counter jumping to 14 immediately, but avoiding 16 -> see onUpdate()
...
function menu.buttonControl(row, data)
  if data and not menu.remapControl then
    ...
    menu.remapControl = {
      ...
      allowmouseaxis =
          data[9],
      checklastnonkeyboard = data[10],
      mouseonly = data[11],
      disableremove = data[12],
      isdblclick = data[13],
      mousewheelonly =
          data[14]
    }
...
function menu.buttonResetControl(row, data)
  if data and not menu.remapControl then
    ...
    if menu.remapControl.controltype == "functions" then
      local definingcontrol = menu.defaultcontrols[menu.remapControl.controltype][menu.remapControl.controlcode]
          .definingcontrol
      conflicts = menu.getDefaultControlConflicts(menu.defaultcontrols[definingcontrol[1]][definingcontrol[2]], conflicts)
    else
      conflicts = menu.getDefaultControlConflicts(
        menu.defaultcontrols[menu.remapControl.controltype][menu.remapControl.controlcode], conflicts)
    end
...
function menu.checkboxControlDblClick(data, checked)
  for i, input in ipairs(menu.controls[data.controltype][data.controlcode]) do
    if (input[1] == data.inputtype) and (input[2] == data.inputcode) then
      if checked then
        menu.controls[data.controltype][data.controlcode][i][2] = menu.controls[data.controltype][data.controlcode][i]
            [2] + 1
      else
        menu.controls[data.controltype][data.controlcode][i][2] = menu.controls[data.controltype][data.controlcode][i]
            [2] - 1
...
function menu.buttonInputProfileSave(profile)
  table.insert(menu.history, 1,
    { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = profile.id })
  menu.displayUserQuestion(ReadText(1001, 4858) .. " - \"" .. profile.name .. "\"",
    function() return menu.callbackInputProfileSave(profile) end)
end

function menu.buttonInputProfileRemove(profile, slot)
  table.insert(menu.history, 1,
    { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = profile.id })
  menu.displayUserQuestion(ReadText(1001, 12715) .. " - \"" .. profile.name .. "\"",
    function() return menu.callbackInputProfileRemove(slot) end)
end
...
function menu.buttonOverwriteSave()
  if menu.selectedOption and next(menu.selectedOption) and (menu.selectedOption.titlerow == nil) and (menu.selectedOption.submenu == nil) then
    if menu.currentOption == "saveoffline" then
      table.insert(menu.history, 1,
        { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = filename })
      local savegame = Helper.tableCopy(menu.selectedOption)
      menu.displayUserQuestion(ReadText(1001, 11714),
        function() return menu.callbackSave(savegame, menu.savegameName) end, nil, nil, nil, nil, nil,
        ReadText(1001, 11715))
    elseif menu.selectedOption.empty then
      ...
      menu.displayUserQuestion(menu.selectedOption.isonline and ReadText(1001, 11708) or ReadText(1001, 8971),
        function() return menu.callbackSave(savegame, menu.savegameName) end)
    end
  end
end
...
function menu.buttonOnlineRegister()
  if C.CanOpenWebBrowser() then
    ...
    C.OpenWebBrowser(((menu.onlineData.hasRegisteredGame == false) and ReadText(1001, 8918) or ReadText(1001, 7294)) ..
      apiinfo)
...
function menu.buttonExtensionUISecurityMode()
  table.insert(menu.history, 1,
    { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = "uisecurity" })
  ...
  Helper.addDelayedOneTimeCallbackOnUpdate(function() SetUISafeModeOption(not GetUISafeModeOption()) end, true,
    getElapsedTime() + 0.1)
...
function menu.addSavegameRow(ftable, savegame, name, slot)
  ...
  local nametruncated = TruncateText(name, config.fontBold, Helper.scaleFont(config.font, config.standardFontSize),
    row[3]:getWidth() - Helper.scaleX(config.standardTextOffsetX))
  local mouseovertext = ""
  ...
  local icon = row[3]:createIcon("solid",
    {
      width = row[3]:getWidth(),
      height = height,
      color = Color["icon_transparent"],
      scaling = false,
      mouseOverText =
          mouseovertext
    }):setText(warningicon .. nametruncated,
    (not invalid) and config.standardTextProperties or config.disabledTextProperties)
  row[3].properties.text.font = config.fontBold
  row[3].properties.text.scaling = true
  if invalid then
    icon:setText2(function() return menu.errorSavegame(savegame) end,
      (not invalid) and config.standardTextProperties or config.disabledTextProperties)
    row[3].properties.text2.y = config.standardTextHeight
    row[3].properties.text2.scaling = true
  elseif savegame.modified then
    icon:setText2(ColorText["text_warning"] .. ReadText(1001, 8901) .. "\27X",
      (not invalid) and config.standardTextProperties or config.disabledTextProperties)
    row[3].properties.text2.y = config.standardTextHeight
    row[3].properties.text2.scaling = true
  elseif isonlinesaveinofflineslot then
    icon:setText2(ColorText["text_online_save"] .. ReadText(1001, 11570) .. "\27X",
      (not invalid) and config.standardTextProperties or config.disabledTextProperties)
    row[3].properties.text2.y = config.standardTextHeight
    row[3].properties.text2.scaling = true
  end
  row[4]:setColSpan(2):createText(savegame.error and "" or savegame.time,
    (not invalid) and config.standardTextProperties or config.disabledTextProperties)
...
function menu.createContextMenu()
  ...
  if menu.contextMenuMode == "remap" then
    ...
    menu.contextFrame:setBackground2("gradient_alpha_04",
      { color = Color["frame_background_semitransparent"], width = Helper.viewWidth, height = Helper.viewHeight })
  ...
  elseif menu.contextMenuMode == "editcolor" then
    menu.contextFrame.properties.x = math.min(menu.contextFrame.properties.x,
      Helper.viewWidth - menu.contextMenuData.width - Helper.frameBorder)
    menu.contextFrame.properties.y = math.min(menu.contextFrame.properties.y,
      Helper.viewHeight - height - Helper.frameBorder)
...
function menu.createContextMenuRemap(frame)
  ...
  if menu.contextMenuData.modifier then
    title = config.input.iscontrollermodifier[menu.contextMenuData.modifier] and ReadText(1001, 12747) or
        ReadText(1001, 12647)
  elseif menu.contextMenuData.removemodifier then
    title = config.input.iscontrollermodifier[menu.contextMenuData.modifier] and ReadText(1001, 12749) or
        ReadText(1001, 12649)
  end
  ...
  if (not menu.contextMenuData.modifier) and (not menu.contextMenuData.removemodifier) then
    local row = ftable:addRow(nil, { fixed = true })
    row[1]:setColSpan(5):createText(menu.getControlName(menu.remapControl.controltype, menu.remapControl.controlcode),
      { font = config.fontBold, halign = "center" })
  end

  if not menu.contextMenuData.removemodifier then
    if not menu.contextMenuData.reset then
      local keyname, keyicon = menu.getInputName(menu.contextMenuData.newinput[1], menu.contextMenuData.newinput[2],
        menu.contextMenuData.newinput[3] or 0)
      local newinputname = keyname .. " " .. keyicon
      local row = ftable:addRow(nil, { fixed = true })
      row[1]:setColSpan(5):createText(ReadText(1001, 12673) .. ReadText(1001, 120) .. " " .. newinputname,
        { halign = "center" })
    end
  ...
  elseif menu.contextMenuData.modifier then
    desc = config.input.iscontrollermodifier[menu.contextMenuData.modifier] and ReadText(1001, 12748) or
        ReadText(1001, 12648)
  elseif menu.contextMenuData.removemodifier then
  ...
  for _, conflict in ipairs(menu.contextMenuData.conflicts) do
    ...
    row[1]:setColSpan(5):createText("· " .. menu.getControlName(conflict.control[1], conflict.control[2]),
      { color = (not conflict.mappable) and Color["text_error"] or nil })
  end

  local buttontable = frame:addTable(5,
    {
      tabOrder = 1,
      x = Helper.borderSize,
      y = Helper.borderSize,
  ...
  if ftable:getFullHeight() + buttontable:getFullHeight() + Helper.scaleY(Helper.standardTextHeight) + 2 * Helper.borderSize + Helper.frameBorder > (Helper.viewHeight - frame.properties.y) then
    ftable.properties.maxVisibleHeight = Helper.viewHeight - frame.properties.y - buttontable:getFullHeight() -
        Helper.scaleY(Helper.standardTextHeight) - 2 * Helper.borderSize - Helper.frameBorder
  end
  buttontable.properties.y = ftable:getVisibleHeight() + Helper.scaleY(Helper.standardTextHeight) + 2 * Helper
      .borderSize
...
function menu.buttonContextRemapConfirm()
  if menu.contextMenuData.modifier then
    ...
    for i = 0, math.pow(2, #config.input.modifiers) - 1 do
      ...
      menu.fixInputConflicts({ menu.contextMenuData.newinput[1], unmodified + offset, menu.contextMenuData.newinput[3] },
        true)
    end
    SaveInputSettings(menu.controls.actions, menu.controls.states, menu.controls.ranges)

    menu.remapInputInternal(menu.contextMenuData.newinput[1], menu.contextMenuData.newinput[2],
      menu.contextMenuData.newinput[3])
  elseif menu.contextMenuData.removemodifier then
    menu.checkForModifier(menu.contextMenuData.removemodifier[1], false)
    SaveInputSettings(menu.controls.actions, menu.controls.states, menu.controls.ranges)

    C.UnmapModifierButton(menu.contextMenuData.removemodifier[1], menu.contextMenuData.removemodifier[2],
      menu.contextMenuData.removemodifier[3], false)
...
function menu.createContextMenuDirectInput(frame)
  ...
  local row = ftable:addRow(nil, { fixed = true })
  row[2]:setColSpan(3):createText(menu.getControlName(menu.remapControl.controltype, menu.remapControl.controlcode),
    { font = config.fontBold, halign = "center" })

  local currentinputname = ReadText(1001, 12709)
  if menu.remapControl.oldinputtype ~= -1 then
    local keyname, keyicon = menu.getInputName(menu.remapControl.oldinputtype, menu.remapControl.oldinputcode,
      menu.remapControl.oldinputsgn)
  ...
  row[4]:createText(function()
    return menu.nameNewAssignment(menu.remapControl.nokeyboard,
      menu.remapControl.mouseonly or menu.remapControl.mousewheelonly)
  end)
...
function menu.createContextMenuRemoveControllerInput(frame)
  ...
  local row = ftable:addRow(nil, { fixed = true })
  row[1]:setColSpan(5):createText(ReadText(1001, 12672) .. ReadText(1001, 120) .. " " .. currentinputname,
    { halign = "center" })
  ...
  row[4].handlers.onClick = function()
    menu.remapControl = nil; menu.closeContextMenu()
  end
...
function menu.buttonContextRemoveControllerInputConfirm()
  if menu.contextMenuData.removed then
    ...
    menu.remapInput(menu.contextMenuData.newinput[1], menu.contextMenuData.newinput[2], menu.contextMenuData.newinput[3],
      true)
  end
  menu.closeContextMenu()
end
...
function menu.createContextMenuModified(frame)
  local ftable = frame:addTable(6,
    { tabOrder = 1, width = menu.contextMenuData.width, x = 3 * Helper.borderSize, y = 3 * Helper.borderSize, defaultInteractiveObject = true })
  ...
  row[1]:setColSpan(6):createText(menu.contextMenuData.thirdparty and ReadText(1001, 9713) or ReadText(1001, 9716),
    Helper.headerRowCenteredProperties)

  local row = ftable:addRow(false, { fixed = true })
  row[1]:setColSpan(6):createText(
    ReadText(1001, 9717) .. ((not menu.contextMenuData.thirdparty) and ("\n\n" .. ReadText(1001, 9718)) or ""),
    { wordwrap = true })

  ftable:addEmptyRow()

  local row = ftable:addRow(true, { fixed = true })
  row[1]:createCheckBox(function() return menu.contextMenuData.saveOption == true end,
  ...
  row[6]:createButton({ helpOverlayID = "modified_client_confirm", helpOverlayText = " ", helpOverlayHighlightOnly = true })
      :setText(ReadText(1001, 14), { halign = "center" })
...
function menu.createContextMenuUISecurity(frame)
  ...
  row[5].handlers.onClick = function()
    __CORE_DETAILMONITOR_USERQUESTION[menu.contextMenuMode] = menu.contextMenuData.saveOption; menu.closeContextMenu()
  end
  ...
  row[6].handlers.onClick = function()
    Helper.addDelayedOneTimeCallbackOnUpdate(
      function()
        __CORE_DETAILMONITOR_USERQUESTION[menu.contextMenuMode] = menu.contextMenuData.saveOption; SetUISafeModeOption(false)
      end, true, getElapsedTime() + 0.1); menu.displayInit(ReadText(1001, 409))
  end
...
function menu.createContextMenuVentureExtension(frame)
  ...
  row[2].handlers.onClick = function() menu.contextMenuData.saveOption = not menu.contextMenuData.saveOption end
  row[5]:createButton({ helpOverlayID = "venture_extension_download_confirm", helpOverlayText = " ", helpOverlayHighlightOnly = true })
      :setText(ReadText(1001, 2617), { halign = "center" })
  row[5].handlers.onClick = function()
  ...
  row[6]:createButton({ helpOverlayID = "venture_extension_download_cancel", helpOverlayText = " ", helpOverlayHighlightOnly = true })
      :setText(ReadText(1001, 2618), { halign = "center" })
  row[6].handlers.onClick = function()
    if menu.contextMenuData.saveOption then OnlineSetVentureConfig("disable_popup", true) end
    menu.closeContextMenu()
  end
...
function menu.createContextMenuFirstGame(frame)
  ...
  local row = ftable:addRow(false, { fixed = true })
  row[1]:setColSpan(3):createText(hastimelines and ReadText(1001, 12716) or ReadText(1001, 12720),
    Helper.headerRowCenteredProperties)
...
function menu.createContextMenuEditColor(frame)
  ...
  row[1]:createIcon("solid",
    {
      color = function() return menu.getColorMapColor(menu.contextMenuData.color) end,
      height = Helper.scaleY(config
        .standardTextHeight) + Helper.borderSize,
      scaling = false
    })
  row[2]:setColSpan(2):createSliderCell({
    ...
    max = 255,
    start = menu
        .contextMenuData.color.r,
    step = 1,
    hideMaxValue = true
  }):setText(ReadText(1001, 12604))
  ...
  row[2]:setColSpan(2):createSliderCell({
    ...
    max = 255,
    start = menu
        .contextMenuData.color.g,
    step = 1,
    hideMaxValue = true
  }):setText(ReadText(1001, 12605))
  ...
  row[2]:setColSpan(2):createSliderCell({
    ...
    max = 255,
    start = menu
        .contextMenuData.color.b,
    step = 1,
    hideMaxValue = true
  }):setText(ReadText(1001, 12606))
  ...
  row[2]:setColSpan(2):createSliderCell({
    ...
    max = 255,
    start = menu
        .contextMenuData.color.a,
    step = 1,
    hideMaxValue = true
  }):setText(ReadText(1001, 12607))
  ...
  row[2]:setColSpan(2):createSliderCell({
    ...
    max = 1,
    start = menu
        .contextMenuData.color.glow,
    step = 0.01,
    hideMaxValue = true
  }):setText(ReadText(1001, 12608))
...
function menu.buttonConfirmColor()
  C.SetColorMapDefinition(menu.contextMenuData.colorid, Helper.ffiColor(menu.contextMenuData.color),
    menu.contextMenuData.color.glow)
  local coloridx = menu.colorLibSettings.colorIndices[menu.contextMenuData.colorid]
...
function menu.displayControlRow(ftable, controlsgroup, controltype, code, context, mouseovertext, mappable,
                                allowmouseaxis, first, checklastnonkeyboard, compassmenusupport, mouseonly,
                                mousewheelonly)
  local buttons = {}
  ...
  if controltype == "functions" then
    ...
    if type(inputs) == "table" then
      local displayed = 0
      for _, input in ipairs(inputs) do
        buttons, compassmenubutton, displayed = menu.getMappedButtons(buttons, compassmenubutton, displayed, input,
          mappable)
      end
    end
  else
  ...
  if not first then
    ...
    row[2]:setColSpan(7):createText(" ",
      { fontsize = 1, height = Helper.scaleY(config.standardTextHeight) / 2 - 2 * Helper.borderSize, scaling = false })
  end
  ...
  for i = 1, numbuttons do
          ...
          if menu.mappedmousebuttons[buttons[i].input2] then
            ...
            if code == 130 then
              if menu.mappedmousebuttons.targetinteract[buttons[i].input2] then
                mouseovertext = mouseovertext ..
                    ...
                    text ..
                    " (" ..
                    ReadText(1026, 2688) .. ")" .. ReadText(1001, 120) .. " " .. menu.getControlName("states", 132)
                showboth = true
              end
            ...
            for _, control in ipairs(menu.mappedmousebuttons[buttons[i].input2]) do
              mouseovertext = mouseovertext ..
                  ...
                  " (" ..
                  ReadText(1026, 2689) ..
                  ")" .. ReadText(1001, 120) .. " " .. menu.getControlName(control[1], control[2])
            end
          end
        else
          ...
          elseif buttons[i].input1 == 19 then
            ...
            elseif menu.mappedmousebuttons.targetselect[buttons[i].input2] then
              ...
              mouseovertext = mouseovertext ..
                  "\n· " ..
                  text ..
                  " (" .. ReadText(1026, 2688) .. ")" .. ReadText(1001, 120) .. " " .. menu.getControlName("states", 130)
              mouseovertext = mouseovertext ..
                  "\n· " .. text .. " (" .. ReadText(1026, 2689) .. ")" .. ReadText(1001, 120) .. " " .. name
            elseif menu.mappedmousebuttons.targetinteract[buttons[i].input2] then
              ...
              mouseovertext = mouseovertext ..
                  "\n· " ..
                  text ..
                  " (" .. ReadText(1026, 2688) .. ")" .. ReadText(1001, 120) .. " " .. menu.getControlName("states", 131)
              mouseovertext = mouseovertext ..
                  "\n· " .. text .. " (" .. ReadText(1026, 2689) .. ")" .. ReadText(1001, 120) .. " " .. name
            end
          end
    ...
    if buttons[i] and (buttons[i].notmappable ~= nil) then
      if buttons[i].keyicon then
        local iconwidth = C.GetTextWidth(" " .. buttons[i].keyicon, config.font,
          Helper.scaleFont(config.font, config.standardFontSize))
        text = TruncateText(text, config.font, Helper.scaleFont(config.font, config.standardFontSize),
      ...
      if buttons[i] then
        row[3].handlers.onClick = function()
          return menu.buttonControl(row.index,
            { controltype, code, buttons[i].input1, buttons[i].input2, buttons[i].input3, 3, not mappable, context,
              allowmouseaxis, checklastnonkeyboard, mouseonly, mouseonly and (numbuttons == 1), isdblclick,
              mousewheelonly })
        end
        ...
        row[3].handlers.onClick = function()
          return menu.buttonControl(row.index,
            { controltype, code, -1, -1, 0, 3, not mappable, context, allowmouseaxis, checklastnonkeyboard, mouseonly,
              mouseonly and (numbuttons == 1), isdblclick, mousewheelonly })
        end
    ...
    -- remove
    row[4]:createButton({
      active = ((buttons[i] and (buttons[i].notmappable == nil)) == true) and
          ((not mouseonly) or (numbuttons > 1)),
    ...
    if buttons[i] then
      row[4].handlers.onClick = function()
        return menu.buttonRemoveControl(row.index,
          { controltype, code, buttons[i].input1, buttons[i].input2, buttons[i].input3, 4, not mappable, context,
            allowmouseaxis, checklastnonkeyboard })
      end
    end

    -- add
    if i == 1 then
      row[5]:createButton({ width = config.standardTextHeight, mouseOverText = ReadText(1026, 2678) }):setText("+",
        { halign = "center", x = 0 })
      row[5].handlers.onClick = function()
        return menu.buttonAddControl(row.index + (next(buttons) and numbuttons or 0),
          { controltype, code, -1, -1, 0, 3, not mappable, context, allowmouseaxis, nil, mouseonly })
      end
    end
    ...
    if i == 1 then
      ...
      row[6]:createButton({
        active = not isdefault,
        width = config.standardTextHeight,
        mouseOverText = ReadText(1026,
          2679)
        ...
        if showcompassmenuoption then
          ...
          for i = 1, 8 do
            ...
            if menu.mappedcompassmenuoptions[30][i] then
              text2 = menu.getControlName(menu.mappedcompassmenuoptions[30][i][1][1],
                menu.mappedcompassmenuoptions[30][i][1][2])
            ...
            table.insert(options,
              {
                id = "30:" .. i,
                text = ColorText["compass_1_ring_pointer"] ..
                    " " .. string.format("\27[input_device_radial_1_%d]\27X", i) .. ReadText(1030, 10 + i),
                icon = "",
                text2 =
                    text2,
          ...
          for i = 1, 8 do
            ...
            if menu.mappedcompassmenuoptions[31][i] then
              text2 = menu.getControlName(menu.mappedcompassmenuoptions[31][i][1][1],
                menu.mappedcompassmenuoptions[31][i][1][2])
            ...
            table.insert(options,
              {
                id = "31:" .. i,
                text = ColorText["compass_2_ring_pointer"] ..
                    " " .. string.format("\27[input_device_radial_2_%d]\27X", i) .. ReadText(1030, 10 + i),
                icon = "",
                text2 =
                    text2,
                displayremoveoption = false
          ...
          row[7]:setColSpan(2):createDropDown(options,
            {
              startOption = compassmenubutton and (compassmenubutton.input1 .. ":" .. compassmenubutton.input2) or "30:0",
              textOverride =
                  compassmenubutton and "" or " ",
              text2Override = " "
            }):setText2Properties({ color = Color["text_inactive"] })
          if compassmenubutton then
            row[7].handlers.onDropDownConfirmed = function(_, id)
              return menu.dropdownControl(row.index,
                { controltype, code, compassmenubutton.input1, compassmenubutton.input2, compassmenubutton.input3, 7, false,
                  context, false }, id)
            end
          else
            row[7].handlers.onDropDownConfirmed = function(_, id)
              return menu.dropdownControl(row.index,
                { controltype, code, -1, -1, 0, 7, false, context, false }, id)
            end
          end
          if mouseovertext then
    ...
    elseif mouseonly then
      ...
      row[7].handlers.onClick = function(_, checked)
        return menu.checkboxControlDblClick(
          {
            row = row.index,
            col = 7,
            controltype = controltype,
            controlcode = code,
            inputtype = buttons[i].input1,
            inputcode =
                buttons[i].input2,
            inputsgn = buttons[i].input3
          }, checked)
      end
...
function menu.displayOption(ftable, option, numCols)
  ...
  if (option.display == nil) or option.display() then
    if option.id == "line" then
      ...
      row[2]:setColSpan(numCols - 1):createText(" ",
        {
          fontsize = 1,
          height = (option.lineheight or 1) * Helper.borderSize,
          cellBGColor = option.linecolor or
              Color["row_separator"]
        })
    elseif option.id == "header" then
      ...
      if option.value then
        row[2]:setColSpan(numCols - 4):createText(option.name,
          isselectable and config.standardTextProperties or config.disabledTextProperties)
        ...
        elseif option.valuetype == "confirmation" then
          row[nextCol]:createButton({ active = isselectable, mouseOverText = isselectable and "" or option.inactive_text })
              :setText(option.value, { halign = "center" })
          row[nextCol].handlers.onClick = option.callback
        elseif option.valuetype == "dropdown" then
          ...
          row[nextCol]:createDropDown(options,
            { active = isselectable, startOption = currentOption, textOverride = textOverride or "" }):setTextProperties({
            halign =
                textalign or "left"
          })
        ...
        elseif option.valuetype == "sounddropdown" then
          local options, currentOption, textOverride, textalign = option.value()
          menu.curDropDownOption[option.id] = tostring(currentOption)
          row[nextCol]:setColSpan(numCols - 3):createDropDown(options,
            { active = isselectable, startOption = currentOption, textOverride = textOverride or "" }):setTextProperties({
            halign =
                textalign or "left"
          })
        ...
        elseif option.valuetype == "slidercell" then
          ...
          row[nextCol]:createSliderCell({
            valueColor = isselectable and Color["slider_value"] or
                Color["slider_value_inactive"],
            min = scale.min,
            minSelect = scale.minSelect,
            max = scale.max,
            maxSelect =
                scale.maxSelect,
            start = scale.start,
            ...
            readOnly = scale
                .readOnly
          }):setText(text, { color = isselectable and Color["text_normal"] or Color["text_inactive"] })
          row[nextCol].handlers.onSliderCellChanged = function(_, value) return option.callback(value) end
        else
          row[nextCol]:createText(function()
            local text = option.value()
            return text
          end, isselectable and config.standardTextProperties or config.disabledTextProperties)
          if isselectable then
            row[nextCol].properties.color = function()
              local _, color = option.value()
              return color
            end
          end
        end
        if option.confirmline then
          if (option.confirmline.display == nil) or option.confirmline.display() then
            ...
            row2[nextCol]:setColSpan(numCols - 4):createButton({ active = option.confirmline.pos_selectable }):setText(
              option.confirmline.pos_name, { halign = "center" })
            row2[nextCol].handlers.onClick = option.confirmline.positive
            if option.confirmline.negative then
              row2[numCols - 1]:setColSpan(numCols - 3):createButton({ active = option.confirmline.neg_selectable })
                  :setText(option.confirmline.neg_name, { halign = "center" })
        ...
        if option.prefixicon then
          ...
          if icon then
            ...
            row[1]:createIcon(icon,
              { color = color, width = iconsize, height = iconsize, x = row[1]:getWidth() - iconsize, scaling = false })
          end
        end
        row[2]:setColSpan(numCols - 1):createText(option.name,
          isselectable and config.standardTextProperties or config.disabledTextProperties)
        if option.wordwrap then
          row[2].properties.wordwrap = option.wordwrap
        end
...
function menu.getControlsData()
  menu.controls = {
    ["actions"] = GetInputActionMap(),
    ["states"] = GetInputStateMap(),
    ["ranges"] = GetInputRangeMap(),
    ["functions"] = config.input.controlFunctions
  }
...
function menu.getDefaultControlsData()
  menu.defaultcontrols = {
    ["actions"] = GetInputActionMap(true),
    ["states"] = GetInputStateMap(true),
    ["ranges"] =
        GetInputRangeMap(true),
    ["functions"] = config.input.controlFunctions
  }
end
...
function menu.getInputName(source, code, signum)
  ...
  elseif source >= 30 and source <= 31 then
    ...
    return
        ((source == 30) and ColorText["compass_1_ring_pointer"] or ColorText["compass_2_ring_pointer"]) ..
        string.format("\27[input_device_radial_%d_%d]\27X", source - 29, code) .. ReadText(1030, code), ""
  else
...
function menu.submenuHandler(optionParameter)
  ...
  elseif optionParameter == "timelines_reset" then
    menu.displayUserQuestion(ReadText(1001, 12622), function() return menu.callbackResetTimelines() end, nil, nil, nil,
      nil, nil, ReadText(1001, 12623))
  ...
  elseif optionParameter == "exit" then
    menu.displayUserQuestion(ReadText(1001, 2645), function() return menu.callbackExit(false) end, nil, nil, nil, nil,
      nil, Helper.isOnlineGame() and ReadText(1001, 11710) or nil)
...
function menu.openSubmenu(optionParameter, selectedOption)
  ...
  table.insert(menu.history, 1,
    { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = selectedOption })
    ...
    if menu.remapControl.modifiersource[newinputtype] then
      ...
      if #conflicts == 0 then
        ...
        menu.contextMenuData = {
          ...
          conflicts =
              conflicts,
          newinput = { newinputtype, newinputcode, newinputsgn },
          modifier = menu.remapControl.modifier
        }
...
function menu.checkForConflicts(newinputtype, newinputcode, newinputsgn, checkall)
  ...
  if checkall then
    for k, controlsorder in pairs(config.input.controlsorder) do
      returnvalue = menu.checkForConflictsInternal(controlsorder, returnvalue, newinputtype, newinputcode, newinputsgn,
        checkall)
    end
  else
    returnvalue = menu.checkForConflictsInternal(menu.controlsorder, returnvalue, newinputtype, newinputcode, newinputsgn,
      checkall)
  end
  return returnvalue
end
                ...
                for i, input in ipairs(inputs) do
                  if menu.checkInput(inputs, i, input, newinput, true) then
                    table.insert(returnvalue,
                      { control = control, mappable = controlgroup.mappable or newinputtype ~= 1 })
                    found = true
                  end
                end
              end
      ...
      if next(menu.savegames) then
        ...
        if entry then
          if entry.invalidpatches or entry.invalidversion or entry.invalidgameid then
            ...
            local timestamp = ConvertTimeString(entry.playtime,
              "%d" .. ReadText(1001, 104) .. " %H" .. ReadText(1001, 102) .. " %M" .. ReadText(1001, 103))
            ...
            return name ..
                " - " ..
                entry.time ..
                (entry.modified and (ColorText["text_warning"] .. " (" .. ReadText(1001, 8901) .. ")\27X") or "") ..
                "\n(" ..
                entry.playername ..
                " - " ..
                money ..
                " - " .. timestamp .. " - " .. ReadText(1001, 2655) .. ReadText(1001, 120) .. " " .. entry.version .. ")"
          end
        else
          return ReadText(1001, 2614)
        end
...
function menu.nameExtensionSettingEnabled()
  return (menu.selectedExtension.isworkshop and menu.selectedExtension.sync) and ReadText(1001, 4832) or
      ReadText(1001, 4825)
end
...
function menu.nameOnlineSeason()
  ...
  elseif menu.onlinesave then
    ...
    if state ~= 0 then
      local basename = entry and (menu.isStartmenu and ReadText(1001, 11717) or ReadText(1001, 11572)) or
          ReadText(1001, 11300)
      if state == 1 then
    ...
    if menu.currentOption == "load" then
      ...
      elseif menu.selectedOption.invalidversion then
        error = error ..
            ReadText(1001, 4885) ..
            ReadText(1001, 120) ..
            " " .. menu.selectedOption.version                    -- Unknown game version required:
      elseif menu.selectedOption.invalidpatches then
        error = ReadText(1001, 2685) ..
        ReadText(1001, 120)                                       -- Missing, old or disabled extensions:
  ...
  if menu.userQuestion then
    if menu.userQuestion.timer and menu.userQuestion.gfxDone then
      return menu.userQuestion.question ..
          " (" .. math.floor(menu.userQuestion.timer - getElapsedTime()) .. ReadText(1001, 100) .. ")"
    end
    return menu.userQuestion.question
  end
  return ""
  ...
  if menu.selectedOption and (type(menu.selectedOption) == "table") then
    ...
    if menu.selectedOption.isworkshop then
      info = info ..
          "\n" ..
          ReadText(1001, 4824) ..
          ReadText(1001, 120) .. " " .. (menu.selectedOption.sync and ReadText(1001, 2617) or ReadText(1001, 2618))
    end
...
function menu.valueGameAutosaveInterval()
  ...
  for i, entry in ipairs(settings) do
    table.insert(options,
      {
        id = entry,
        text = string.format("%d - %d %s", intervalinfo.mintime * entry / 60,
          intervalinfo.maxtime * entry / 60, ReadText(1001, 103)),
        icon = "",
        displayremoveoption = false
      })
  end
...
function menu.valueGameEnemyAttack()
  ...
  for i = 0, n - 1 do
    table.insert(options,
      { id = ffi.string(buf[i].id), text = ffi.string(buf[i].name), icon = "", displayremoveoption = false })
  end
...
function menu.valueGameStartmenuBackground()
  ...
  for i = 0, n - 1 do
    ...
    local extensionname = (buf[i].extensionidx == 0) and ReadText(1021, 65) or
        ffi.string(C.GetExtensionName(buf[i].extensionidx))

    table.insert(scenes,
      {
        id = id,
        text = extensionname,
        counter = extensioncounts[buf[i].extensionidx],
        extensionidx = buf[i]
            .extensionidx,
        isdefault = buf[i].isdefault
      })
  end
  ...
  for _, scene in ipairs(scenes) do
    if scene.isdefault then
      ...
      local text = scene.text ..
          ((extensioncounts[scene.extensionidx] > 1) and (" " .. ReadText(20402, scene.counter)) or "")
...
function menu.valueGfxAdaptiveSampling()
  ...
  for _, entry in ipairs(settings) do
    table.insert(options,
      { id = entry[1], text = entry[2] or Helper.roundStr(entry[1], 2), icon = "", displayremoveoption = false })
  end
...
function menu.valueGfxDLSS()
  ...
  for i, entry in ipairs(settings) do
    table.insert(options,
      { id = entry[1], text = entry[2], icon = "", displayremoveoption = false, mouseovertext = entry[3] or "" })
  end
...
function menu.valueGfxGPU()
  ...
  for i = 1, numgpus do
    table.insert(options,
      {
        id = i,
        text = ffi.string(C.GetGPUNiceName(i)),
        icon = "",
        displayremoveoption = false,
        active = C
            .IsGPUCompatible(i)
    ...
    table.insert(options,
      {
        ...
        icon = "",
        displayremoveoption = false,
        active = (entry.accessibility == nil) or
...
function menu.valueGfxResolution()
  ...
  for i, resolution in ipairs(resolutions) do
    table.insert(options,
      {
        id = resolution.width .. ":" .. resolution.height,
        text = resolution.width ..
            ReadText(1001, 42) .. resolution.height,
        icon = "",
        displayremoveoption = false
      })
  end
    ...
    table.insert(options,
      {
        ...
        active = entry.active,
        mouseovertext =
            entry.active and "" or (ColorText["text_error"] .. ReadText(1026, 2627))
      })
  end
    ...
    table.insert(options,
      {
        ...
        active = entry.active,
        mouseovertext =
            entry.active and (entry[3] or "") or (ColorText["text_error"] .. ReadText(1026, 2627))
      })
  end
...
function menu.valueInputTrackerMode()
  ...
  for i, entry in ipairs(settings) do
    table.insert(options,
      { id = entry[1], text = entry[2], icon = "", displayremoveoption = false, mouseovertext = entry[3] })
  end
...
function menu.valueOnlinePreferredLanguage()
  ...
  for _, language in ipairs(menu.languagedata) do
    table.insert(options,
      { id = language.id, text = language.name, icon = "", displayremoveoption = false, font = language.font })
  end
...
function menu.valueSfxDevice()
  ...
  for i = 0, n - 1 do
    ...
    local availablewidth = menu.table.infoColumnWidth - Helper.scaleX(config.standardTextHeight) -
        Helper.scaleX(config.standardTextOffsetX)
    if length > availablewidth then
      ...
      text = TruncateText(utf8.sub(text, 1, -7), config.font, Helper.scaleFont(config.font, config.standardFontSize),
        availablewidth - suffixlength) .. suffix
    end

    table.insert(options,
      { id = name, text = text, icon = "", displayremoveoption = false, mouseovertext = mouseovertext })
  end
...
function menu.loadGameCallback(filename, checked)
  ...
  for ware, waredata in Helper.orderedPairs(playerinventory) do
    local isbraneitem, isoperationvolatile, isseasonvolatile, isventureuploadallowed = GetWareData(ware, "isbraneitem",
      "isoperationvolatile", "isseasonvolatile", "isventureuploadallowed")
    if isbraneitem then
  ...
  if (not checked) and (not menu.isStartmenu) and Helper.isOnlineGame() and hasnotuploadeditems then
    table.insert(menu.history, 1,
      { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = filename })
    menu.displayUserQuestion(ReadText(1001, 2604) .. " - " .. ReadText(1001, 7720),
      function() return menu.loadGameCallback(filename, true) end, nil, nil, nil, nil, nil, ReadText(1001, 11707))
  else
    Helper.addDelayedOneTimeCallbackOnUpdate(function() LoadGame(filename) end, true, getElapsedTime() + 0.1)
    menu.displayInit()
  end
...
function menu.callbackGameHUDScale(id, option)
  if option ~= menu.curDropDownOption[id] then
    ...
    table.insert(menu.history, 1,
      { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = "hudscale" })
    __CORE_GAMEOPTIONS_RESTORE = true
    __CORE_GAMEOPTIONS_RESTOREINFO.optionParameter = nil
    __CORE_GAMEOPTIONS_RESTOREINFO.history = menu.history
...
function menu.callbackGameMenuWidthScaleConfirm()
  if menu.newMenuWidthScale ~= Helper.round(C.GetMenuWidthScale(), 2) then
    table.insert(menu.history, 1,
      { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = "menuwidthscale" })
    __CORE_GAMEOPTIONS_RESTORE = true
    __CORE_GAMEOPTIONS_RESTOREINFO.optionParameter = nil
    __CORE_GAMEOPTIONS_RESTOREINFO.history = menu.history
    Helper.addDelayedOneTimeCallbackOnUpdate(function() C.SetMenuWidthScale(menu.newMenuWidthScale) end, true,
      getElapsedTime() + 0.1)
    menu.displayInit(ReadText(1001, 409))
  end
end
...
function menu.callbackGameStartmenuBackground(id, option)
  if option ~= menu.curDropDownOption[id] then
    ...
    table.insert(menu.history, 1,
      {
        optionParameter = menu.currentOption,
        topRow = GetTopRow(menu.optionTable),
        selectedOption =
...
function menu.callbackGameStartmenuBackgroundConfirm()
  table.insert(menu.history, 1,
    { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = "startmenu_background" })
  __CORE_GAMEOPTIONS_RESTORE = true
  __CORE_GAMEOPTIONS_RESTOREINFO.optionParameter = nil
  __CORE_GAMEOPTIONS_RESTOREINFO.history = menu.history
  NewGame("startmenu")
...
function menu.callbackGameUIScaleConfirm()
  if menu.newUIScale ~= Helper.round(C.GetUIScaleFactor(), 1) then
    table.insert(menu.history, 1,
      { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = "uiscale" })
    __CORE_GAMEOPTIONS_RESTORE = true
    __CORE_GAMEOPTIONS_RESTOREINFO.optionParameter = nil
    __CORE_GAMEOPTIONS_RESTOREINFO.history = menu.history
    Helper.addDelayedOneTimeCallbackOnUpdate(function() C.SetUIScaleFactor(menu.newUIScale) end, true,
      getElapsedTime() + 0.1)
    menu.displayInit(ReadText(1001, 409))
  end
end
...
function menu.callbackGfxAA(id, option)
  if option ~= menu.curDropDownOption[id] then
    ...
    table.insert(menu.history, 1,
      { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = "antialias" })
    __CORE_GAMEOPTIONS_RESTORE = true
    __CORE_GAMEOPTIONS_RESTOREINFO.history = menu.history
...
function menu.callbackGfxAACancel()
  ...
  C.SetMultipleGfxModes(ffi.string(C.GetAAOption(true)), upmode, C.GetDLSSOption(true),
    ffi.string(C.GetDLSSModeOption(true)))
...
function menu.callbackGfxAdapter(id, option)
  if option ~= menu.curDropDownOption[id] then
    ...
    table.insert(menu.history, 1,
      { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = "adapter" })
    __CORE_GAMEOPTIONS_RESTORE = true
    __CORE_GAMEOPTIONS_RESTOREINFO.optionParameter = nil
    __CORE_GAMEOPTIONS_RESTOREINFO.history = menu.history
  end
...
function menu.callbackGfxDLSS(id, option)
  if option ~= menu.curDropDownOption[id] then
    ...
    table.insert(menu.history, 1,
      { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = "dlss" })
    __CORE_GAMEOPTIONS_RESTORE = true
    __CORE_GAMEOPTIONS_RESTOREINFO.history = menu.history
    __CORE_GAMEOPTIONS_RESTOREINFO.questionParameter = {
      question = ReadText(1001, 2602),
...
function menu.callbackGfxDLSSMode(id, option)
  if option ~= menu.curDropDownOption[id] then
    ...
    table.insert(menu.history, 1,
      { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = "dlssmode" })
    __CORE_GAMEOPTIONS_RESTORE = true
    __CORE_GAMEOPTIONS_RESTOREINFO.history = menu.history
    __CORE_GAMEOPTIONS_RESTOREINFO.questionParameter = {
      question = ReadText(1001, 2602),
...
function menu.callbackGfxFullscreen(id, option)
  if option ~= menu.curDropDownOption[id] then
    ...
    table.insert(menu.history, 1,
      { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = "fullscreen" })
    if C.IsVRMode() then
      menu.displayUserQuestion(ReadText(1001, 2602), menu.callbackGfxFullscreenConfirm, menu.callbackGfxFullscreenCancel,
        15.9, true)
    else
      __CORE_GAMEOPTIONS_RESTORE = true
      __CORE_GAMEOPTIONS_RESTOREINFO.history = menu.history
...
function menu.callbackGfxResolution(id, option)
  if option ~= menu.curDropDownOption[id] then
    ...
    table.insert(menu.history, 1,
      { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = "resolution" })
    if C.IsVRMode() then
      menu.displayUserQuestion(ReadText(1001, 2602), menu.callbackGfxResolutionConfirm, menu.callbackGfxResolutionCancel,
        15.9, true)
    else
      __CORE_GAMEOPTIONS_RESTORE = true
      __CORE_GAMEOPTIONS_RESTOREINFO.history = menu.history
...
function menu.callbackGfxFSR1(id, option)
  if option ~= menu.curDropDownOption[id] then
    ...
    table.insert(menu.history, 1,
      { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = "fsr1" })
    __CORE_GAMEOPTIONS_RESTORE = true
    __CORE_GAMEOPTIONS_RESTOREINFO.history = menu.history
    __CORE_GAMEOPTIONS_RESTOREINFO.questionParameter = {
      question = ReadText(1001, 2602),
...
function menu.displayOptions(optionParameter)
  ...
  local ftable = frame:addTable(5,
    { tabOrder = 1, x = menu.table.x, y = menu.table.y, width = menu.table.width, maxVisibleHeight = menu.table.height })
  ...
  ftable:setDefaultComplexCellProperties("button", "text",
  ...
  ftable:setDefaultComplexCellProperties("dropdown", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize })
  ftable:setDefaultCellProperties("slidercell", { height = config.standardTextHeight })
  ftable:setDefaultComplexCellProperties("slidercell", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize })
  ...
  if options.warning then
    ...
    local row = ftable:addRow(false, { fixed = true })
    row[1]:setColSpan(5):createText(function()
      local text = options.warning()
      return text
    end, config.warningTextProperties)
    if warningFont then
...
function menu.displayOptionsInfo(optionParameter)
  ...
  local optiontable = frame:addTable(5,
    { tabOrder = 1, x = menu.table.x, y = offsety, width = menu.table.widthWithExtraInfo, maxVisibleHeight = height })
  optiontable:setColWidth(1, menu.table.arrowColumnWidth, false)
  optiontable:setColWidthPercent(3, 10)
  optiontable:setColWidth(4,
    0.1 * menu.table.widthWithExtraInfo - Helper.scaleY(config.infoTextHeight) - Helper.borderSize, false)
  ...
  local infotable = frame:addTable(1,
    { tabOrder = 0, x = offsetx, y = offsety, width = width, maxVisibleHeight = height })

  local row = infotable:addRow(false, { bgColor = Color["optionsmenu_cell_background"] })
  row[1]:createText(menu.infoHandler,
    {
      scaling = false,
      width = width,
      height = height,
      wordwrap = true,
      fontsize = Helper.scaleFont(config.font,
        config.infoFontSize)
    })
...
function menu.displayNewGame(createAsServer, displayTimelinesScenarios, displayTutorials)
  ...
  local optiontable = frame:addTable(2,
    { tabOrder = 1, x = menu.table.x, y = offsety, width = menu.table.widthWithExtraInfo, maxVisibleHeight = height })
  optiontable:setColWidth(1, menu.table.arrowColumnWidth, false)
      ...
      if module.available or showunavailable then
        ...
        if available and module.unlocked then
          if recommendedstarts[module.id] then
            local iconsize = Helper.scaleY(config.infoTextHeight)
            row[1]:createIcon("menu_recommended",
              {
                color = Color["gamestart_recommended"],
                mouseOverText = ReadText(1026, 2680),
                width = iconsize,
                height =
                    iconsize,
                x = row[1]:getWidth() - iconsize,
                scaling = false
              })
            mouseovertext = ReadText(1026, 2680)
          end
        ...
        if module.tutorial then
          if tonumber(ffi.string(C.GetUserData(module.id .. "_completed"))) == 1 then
            local iconsize = Helper.scaleY(config.infoTextHeight)
            row[1]:createIcon("widget_tick_01",
              {
                color = Color["tutorial_completed"],
                width = iconsize,
                height = iconsize,
                x = row[1]:getWidth() -
                    iconsize,
        ...
        row[2]:createText(name .. recommended,
          available and module.unlocked and config.standardTextProperties or config.disabledTextProperties)
        if mouseovertext then
          row[2].properties.mouseOverText = mouseovertext
        end
      end
  ...
  local infotable = frame:addTable(1,
    { tabOrder = 3, x = offsetx, y = offsety, width = width, maxVisibleHeight = maxVisibleHeight, highlightMode = "off" })
  ...
  if menu.selectedOption then
    ...
    if menu.selectedOption.requirement ~= "" then
      local canopenstore = (menu.selectedOption.extensionid ~= "") and
          (not C.HasExtension(menu.selectedOption.extensionid, menu.selectedOption.isextensionpersonal)) and
          (IsSteamworksEnabled() or (C.IsGOGVersion() and C.CanOpenWebBrowser()))
      if canopenstore then
        local iconsize = Helper.scaleY(Helper.standardButtonHeight)
        row[1]:createButton({
          bgColor = Color["icon_error"],
          highlightColor = canopenstore and
              Color["button_highlight_default"] or Color["row_background"]
        }):setText(menu.selectedOption.requirement,
          { x = config.infoTextOffsetX, font = config.fontBold }):setIcon("mm_externallink",
        ...
        if storeicon ~= "" then
          row[1]:setIcon2(storeicon,
            { scaling = false, x = width - 2 * iconsize - Helper.borderSize, height = iconsize, width = iconsize })
        end
        row[1].handlers.onClick = function() return menu.buttonOpenStore(menu.selectedOption.extensionsource) end
      else
        row[1]:createText(menu.selectedOption.requirement,
          {
            x = config.infoTextOffsetX,
            y = (Helper.standardButtonHeight - Helper.standardTextHeight) / 2,
            font = config
                .fontBold,
            cellBGColor = Color["icon_error"],
            minRowHeight = Helper.standardButtonHeight
          })
      end
    else
      row[1]:createText(ReadText(1001, 11732) .. ReadText(1001, 120) .. " " .. menu.selectedOption.typename,
        {
          mouseOverText = menu.selectedOption.typedescription,
          x = config.infoTextOffsetX,
          y = (Helper.standardButtonHeight - Helper.standardTextHeight) /
              2,
          font = config.fontBold,
          cellBGColor = Color["optionsmenu_cell_background_icon"],
          minRowHeight = Helper
    ...
    local descriptiontext = GetTextLines(text, Helper.standardFont,
      Helper.scaleFont(Helper.standardFont, config.infoFontSize), width - 2 * Helper.scaleX(config.infoTextOffsetX))
    if #descriptiontext > numlines then
      -- scrollbar case
      descriptiontext = GetTextLines(text, Helper.standardFont,
        Helper.scaleFont(Helper.standardFont, config.infoFontSize),
        width - 2 * Helper.scaleX(config.infoTextOffsetX) - Helper.scrollbarWidth)
    end
  ...
  local fullheight = infotable:getFullHeight() +
      (hasdescription and 0 or Helper.borderSize) -- if there is no description, the border between icon row and the new empty row has to be accounted for
  if fullheight < infotable.properties.maxVisibleHeight then
    ...
    row[1]:createText("",
      { scaling = false, minRowHeight = infotable.properties.maxVisibleHeight - fullheight, fontsize = 1 })
  end

  local iconwidth = math.floor(0.27 * width) - Helper.borderSize
  local infotable2 = frame:addTable(3,
    {
      tabOrder = 4,
      x = offsetx,
      y = infotable.properties.y + infotable.properties.maxVisibleHeight + Helper.borderSize,
      width =
          width,
      maxVisibleHeight = infoheight - infotable.properties.maxVisibleHeight - Helper.borderSize,
      highlightMode =
      "backgroundcolumn"
    })
  ...
  local passiverows = {}
  if menu.selectedOption then
    local isspecial = menu.selectedOption.customeditor or menu.selectedOption.mapeditor or
        menu.selectedOption.stationeditor
    ...
    row[1]:setColSpan(3):createText(" ",
      { fontsize = 1, height = Helper.borderSize, cellBGColor = Color["row_background_blue"] })

    if IsCheatVersion() then
      local row = infotable2:addRow(true,
        { bgColor = Color["optionsmenu_cell_background"], borderBelow = false, interactive = false })
      row[1]:createText("Gamestart ID:") -- (cheat only)
      row[2]:setColSpan(valuecolspan):createText(ColorText["text_inactive"] .. menu.selectedOption.id,
        { halign = "right" })
    ...
    if menu.selectedOption.custom then
      ...
      if ffi.string(buf[0].state) == "standard" then
        ...
        for macro in string.gmatch(ffi.string(buf[0].options), "[%w_]+") do
          ...
          table.insert(macros,
            { macro = macro, race = race, racename = racename, gender = female and "female" or "male" })
        end
        ...
        for _, entry in ipairs(macros) do
          ...
          if (lastrace == entry.race) and (lastgender == entry.gender) then
            if doublingcount == 0 then
              playermacrooptions[#playermacrooptions].text = playermacrooptions[#playermacrooptions].text ..
                  " " .. ReadText(20402, 1)
              doublingcount = 2
            else
      ...
      if menu.selectedOption.usetimelinesplayercharacter then
        ...
        if playermacro ~= "" then
          ...
          if found then
            ...
            DebugError("Requested timelines player character macro '" ..
              tostring(playermacro) .. "' not found in timelines hub gamestart options!")
          end
        end
      end
    end
    ...
    for i, entry in ipairs(menu.selectedOption.info) do
      ...
      if entry.info == "@name" then
        row = infotable2:addRow(true,
          { bgColor = Color["optionsmenu_cell_background"], borderBelow = false, interactive = false })
        row[1]:createText(ReadText(1021, 8) .. ReadText(1001, 120))
        local gamestartid = menu.selectedOption.id
        row[2]:setColSpan(valuecolspan):createText(
          function()
            local buf = ffi.new("CustomGameStartStringPropertyState[1]"); return ColorText["text_inactive"] ..
                ffi.string(C.GetCustomGameStartStringProperty(gamestartid, "playername", buf))
          end, { halign = "right" })
        table.insert(passiverows, row)
      elseif entry.info == "@player" then
        ...
        if #playermacrooptions > 0 then
          ...
          row[2]:setColSpan(valuecolspan):createDropDown(playermacrooptions,
            { startOption = playermacro, height = Helper.standardTextHeight }):setTextProperties({
            halign = "right",
            x =
                Helper.standardTextOffsetx
          })
          row[2].handlers.onDropDownConfirmed = function(_, id)
            return menu.callbackGamestartPlayerMacro(
              menu.selectedOption.id, "player", id)
          end
        else
          row[1]:createText("")
        end
      elseif entry.info == "@unlock" then
        if not menu.selectedOption.unlocked then
          ...
          row[2]:setColSpan(valuecolspan):createText(ColorText["text_inactive"] .. entry.description,
            { halign = "right" })
          table.insert(passiverows, row)
        end
      ...
      if row then
        ...
        if (playerimage ~= "") or (infoimage ~= "") then
          if i == imageindex then
            imagerow = row
            row[3]:createIcon(
              (playerimage ~= "") and function() return menu.getPlayerMacroIcon(menu.selectedOption) end or infoimage,
              {
                width = iconwidth,
                height = iconwidth,
                scaling = false,
                ...
                y = iconwidth / 2 -
                    Helper.scaleY(config.infoTextHeight) / 2,
                cellBGColor = Color["optionsmenu_cell_background_icon"]
              })
          else
  ...
  buttonrow[1]:setColSpan(3):setBackgroundColSpan(3):createButton({
    active = menu.buttonStartGameActive(),
    height =
        config.standardTextHeight
  }):setText(ReadText(1001, 9902), { halign = "center" })
...
function menu.displayTimelines()
  ...
  local titletable = frame:addTable(5,
    { tabOrder = 2, x = menu.table.x, y = menu.table.y, width = menu.table.width, maxVisibleHeight = menu.table.height })
  titletable:setColWidth(1, menu.table.arrowColumnWidth, false)
  titletable:setColWidth(3, menu.table.infoColumnWidth / 2, false)
  titletable:setColWidth(4, menu.table.infoColumnWidth / 2 - Helper.scaleY(config.infoTextHeight) - Helper.borderSize,
    false)
  ...
  titletable:setDefaultComplexCellProperties("button", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize })
  ...
  if not showCutscene then
    local row = infotable:addRow(false, { bgColor = Color["optionsmenu_cell_background_icon"], fixed = true })
    row[1]:createIcon(timelinesgamestart and timelinesgamestart.image or "gamestart_default",
      { scaling = false, width = width, height = iconheight })
  end
  ...
  if timelinesgamestart then
    ...
    if timelinesgamestart.requirement ~= "" then
      local canopenstore = (timelinesgamestart.extensionid ~= "") and
          (not C.HasExtension(timelinesgamestart.extensionid, timelinesgamestart.isextensionpersonal)) and
          (IsSteamworksEnabled() or (C.IsGOGVersion() and C.CanOpenWebBrowser()))
      if canopenstore then
        local iconsize = Helper.scaleY(Helper.standardButtonHeight)
        row[1]:createButton({
          ...
          highlightColor = canopenstore and
              Color["button_highlight_default"] or Color["row_background"]
        }):setText(timelinesgamestart.requirement,
          { x = config.infoTextOffsetX, font = config.fontBold }):setIcon("mm_externallink",
          { scaling = false, x = width - iconsize, height = iconsize, width = iconsize })
        local storeicon = ""
        if IsSteamworksEnabled() then
        ...
        row[1]:createText(timelinesgamestart.requirement,
          {
            x = config.infoTextOffsetX,
            y = (Helper.standardButtonHeight - Helper.standardTextHeight) / 2,
            font = config
      ...
      row[1]:createText(ReadText(1001, 11732) .. ReadText(1001, 120) .. " " .. timelinesgamestart.typename,
        {
          mouseOverText = timelinesgamestart.typedescription,
          x = config.infoTextOffsetX,
          y = (Helper.standardButtonHeight - Helper.standardTextHeight) /
              2,
          font = config.fontBold,
  ...
  local passiverows = {}
  if timelinesgamestart then
    local isspecial = timelinesgamestart.customeditor or timelinesgamestart.mapeditor or timelinesgamestart
        .stationeditor
    local valuecolspan = 1
    if isspecial then
      valuecolspan = 2
    ...
    if IsCheatVersion() then
      local row = infotable2:addRow(true,
        { bgColor = Color["optionsmenu_cell_background"], borderBelow = false, interactive = false })
      row[1]:createText("Gamestart ID:") -- (cheat only)
      row[2]:setColSpan(valuecolspan):createText(ColorText["text_inactive"] .. timelinesgamestart.id,
        { halign = "right" })
      table.insert(passiverows, row)
    end
    local infostarty = infotable2:getFullHeight()
      ...
      elseif entry.info == "@player" then
        if ffi.string(C.GetUserData("timelines_scenarios_finished")) == "" then
          ...
          if #playermacrooptions > 0 then
            ...
            row[2].handlers.onDropDownConfirmed = function(_, id)
              return menu.dropdownTimelinesCharacter(
                timelinesgamestart, id)
            end
          else
            row[1]:createText("")
          end
      ...
      if row then
        ...
        if (playerimage ~= "") or (infoimage ~= "") then
          if i == imageindex then
            imagerow = row
            row[3]:createIcon(
              (playerimage ~= "") and function() return menu.getPlayerMacroIcon(timelinesgamestart) end or infoimage,
              {
                width = iconwidth,
                height = iconwidth,
                scaling = false,
  ...
  local buttonrow = infotable2:addRow(true, { bgColor = Color["optionsmenu_cell_background"] })
  buttonrow[1]:setColSpan(3):setBackgroundColSpan(3):createButton({
    active = C.IsExtensionEnabled("ego_dlc_timelines",
      false),
    height = config.standardTextHeight
  }):setText(
    (ffi.string(C.GetUserData("timelines_scenarios_finished")) ~= "") and ReadText(1001, 12620) or ReadText(1001, 12619),
    { halign = "center" })
...
function menu.displayExtensions()
  ...
  local titletable = frame:addTable(2,
    { tabOrder = 2, x = menu.table.x, y = menu.table.y, width = menu.table.widthExtraWide, skipTabChange = true })
  titletable:setColWidth(1, menu.table.arrowColumnWidth, false)
  ...
  local optiontable = frame:addTable(7,
    {
      tabOrder = 1,
      x = menu.table.x,
      y = offsety,
      width = menu.table.widthExtraWide - infowidth - Helper.borderSize,
      maxVisibleHeight =
          height
    })
  optiontable:setColWidth(1, menu.table.arrowColumnWidth, false)
  ...
  if IsSteamworksEnabled() then
    ...
    row[6]:createButton({}):setText(function()
        local text = menu.valueExtensionGlobalSync()
        return text
      end,
      {
        fontsize = config.standardFontSize,
        halign = "center",
        color = function()
          local _, color = menu.valueExtensionGlobalSync()
          return color
        end
      })
  ...
  row[6]:createButton({ mouseOverText = ReadText(1001, 12725) }):setText(
    function() return GetUISafeModeOption() and ReadText(1001, 12642) or ReadText(1001, 12641) end,
    { fontsize = config.standardFontSize, halign = "center" })
  ...
  local infotable = frame:addTable(1,
    { tabOrder = 0, x = offsetx, y = offsety, width = infowidth, maxVisibleHeight = height })

  local row = infotable:addRow(false, { bgColor = Color["optionsmenu_cell_background"] })
  row[1]:createText(menu.descriptionExtension,
    {
      scaling = false,
      width = infowidth,
      height = height,
...
function menu.displayExtensionRow(ftable, extension, extensionSetting)
  ...
  row[6]:createButton({}):setText(function() return menu.valueExtensionStatus(extension) end,
    {
      fontsize = config.standardFontSize,
      halign = "center",
      color = function()
        local _, color = menu.valueExtensionStatus(extension); return color
      end
    })
  ...
  row[7].handlers.onClick = function()
    menu.selectedExtension = extension; menu.openSubmenu("extensionsettings", extension.id)
  end
end
...
function menu.displayBonusContent()
  ...
  if owned then
    ...
    local row = infotable:addRow(false, { bgColor = Color["optionsmenu_cell_background"] })
    row[1]:createText(menu.descriptionBonusContent,
      {
        scaling = false,
        width = width,
        height = height,
...
function menu.displayColorLibrary()
  ...
  local numcols = 2
  local titletable = frame:addTable(numcols,
    { tabOrder = 5, x = menu.table.x, y = menu.table.y, width = titletablewidth, skipTabChange = true })
  titletable:setColWidth(1, menu.table.arrowColumnWidth, false)
  ...
  local numcols = 3
  local optiontable = frame:addTable(numcols,
    { tabOrder = 1, x = menu.table.x, y = offsety, width = titletablewidth / 2 })
  optiontable:setColWidth(1, menu.table.arrowColumnWidth, false)
  optiontable:setColWidthPercent(2, 40)
  optiontable:setDefaultCellProperties("dropdown", { height = Helper.scaleY(config.standardTextHeight), scaling = false })
  optiontable:setDefaultComplexCellProperties("dropdown", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize, scaling = true })
  optiontable:setDefaultCellProperties("slidercell",
    { height = Helper.scaleY(config.standardTextHeight), scaling = false })
  optiontable:setDefaultComplexCellProperties("slidercell", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize, scaling = true })
  ...
  row[3].handlers.onDropDownConfirmed = function(_, id)
    if id ~= mode then
      C.SetColorBlindOption(id); menu.refresh()
    end
  end
  ...
  row[3].handlers.onSliderCellDeactivated = function()
    if menu.colorLibSettings.colorblindstrength ~= modestrength then
      C.SetColorBlindOptionStrength(Helper.round(menu.colorLibSettings.colorblindstrength / 100, 2)); menu.refresh()
    end
  end
  ...
  if n > 0 then
    ...
    for i = 0, n - 1 do
      ...
      table.insert(menu.colorLibSettings.colors,
        { id = id, color = color, ispersonal = buf[i].ispersonal, isdeletable = buf[i].isdeletable })
      menu.colorLibSettings.colorIndices[id] = #menu.colorLibSettings.colors
      table.insert(menu.colorLibSettings.colorDropdownOptions,
        { id = id, text = id, icon = "", displayremoveoption = false })
  ...
  colortable:setDefaultComplexCellProperties("button", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize })
  colortable:setDefaultCellProperties("dropdown", { height = config.standardTextHeight })
  colortable:setDefaultComplexCellProperties("dropdown", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize })
  colortable:setDefaultCellProperties("slidercell", { height = config.standardTextHeight })
  colortable:setDefaultComplexCellProperties("slidercell", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize })
  ...
  row[1]:setColSpan(numcols):createEditBox({ height = config.standardTextHeight, defaultText = ReadText(1001, 3250) })
      :setText(menu.colorSearchText, { x = Helper.standardTextOffsetx })
  ...
  for _, entry in ipairs(menu.colorLibSettings.colors) do
    if menu.filterColorDefinition(entry.id, menu.colorSearchText) then
      ...
      row[2]:createButton({
        bgColor = Color["button_background_white"],
        highlightColor = Color
            ["button_highlight_hidden"]
      }):setIcon2("solid",
        { color = function() return menu.getDefinitionColor(entry.id) end }):setIcon("menu_checker",
        {
          scaling = false,
          width = row[2]:getWidth() / 2,
          height = Helper.scaleY(config.standardTextHeight),
          x = row[2]
              :getWidth() / 2
        })
      ...
      if entry.isdeletable then
        row[4]:createButton({ active = function() return (menu.colorLibSettings.colorsUsedCount[entry.id] or 0) == 0 end })
            :setText("x")
        row[4].handlers.onClick = function()
          C.RemoveColorMapColorDefinition(entry.id); menu.refresh()
        end
      end
    end
  end
  ...
  local numcols = 4
  local mappingtable = frame:addTable(numcols,
    {
      tabOrder = 3,
      x = menu.table.x + colortablewidth + Helper.borderSize,
      y = offsety,
  ...
  mappingtable:setDefaultComplexCellProperties("button", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize })
  mappingtable:setDefaultCellProperties("dropdown", { height = config.standardTextHeight })
  mappingtable:setDefaultComplexCellProperties("dropdown", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize })
  mappingtable:setDefaultCellProperties("slidercell", { height = config.standardTextHeight })
  mappingtable:setDefaultComplexCellProperties("slidercell", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize })
  ...
  row[1]:setColSpan(numcols):createEditBox({ height = config.standardTextHeight, defaultText = ReadText(1001, 3250) })
      :setText(menu.mappingSearchText, { x = Helper.standardTextOffsetx })
  ...
  row[4]:createButton({
    height = config.subHeaderTextHeight,
    width = config.subHeaderTextHeight,
    bgColor = menu
        .colorLibSettings.sortByReference and Color["button_background_active"] or nil
  }):setIcon("widget_arrow_down_01")
  row[4].handlers.onClick = function()
    menu.colorLibSettings.sortByReference = not menu.colorLibSettings.sortByReference; menu.refresh()
  end
  ...
  for i, entry in ipairs(menu.colorLibSettings.mappings) do
    ...
    if display then
      ...
      row[2]:createButton({
        ...
        highlightColor = Color
            ["button_highlight_hidden"]
      }):setIcon2("solid", { color = function() return menu.getMappingColor(entry.id) end })
          :setIcon("menu_checker",
            {
              scaling = false,
              width = row[2]:getWidth() / 2,
              height = Helper.scaleY(config.standardTextHeight),
  ...
  buttontable:setDefaultComplexCellProperties("button", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize })
  buttontable:setDefaultCellProperties("dropdown", { height = Helper.scaleY(config.standardTextHeight), scaling = false })
  buttontable:setDefaultComplexCellProperties("dropdown", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize, scaling = true })

  local row = buttontable:addRow(true, { fixed = true })
  row[1]:createEditBox({ height = config.standardTextHeight, description = ReadText(1001, 12610) }):setText(
    menu.colorLibSettings.newColorDefinition, { fontsize = config.standardFontSize, x = Helper.standardTextOffsetx })
  row[1].handlers.onTextChanged = function(_, text) menu.colorLibSettings.newColorDefinition = text end
  row[2]:createButton({
    active = function()
      return (menu.colorLibSettings.newColorDefinition ~= "") and
          (menu.colorLibSettings.colorIndices[menu.colorLibSettings.newColorDefinition] == nil)
    end
  }):setText(
    ReadText(1001, 12611), { halign = "center" })
  row[2].handlers.onClick = function()
    C.AddColorMapColorDefinition(menu.colorLibSettings.newColorDefinition); menu.colorLibSettings.newColorDefinition = ""; menu
        .refresh()
  end
  ...
  row[4]:setColSpan(2):createDropDown(profiles,
    { startOption = "", textOverride = ReadText(1001, 12703), active = n > 0 })
  row[4].handlers.onDropDownConfirmed = function(_, option)
    menu.colorLibSettings.newColorProfileName = menu.colorLibSettings.profilesByFilename[option]; menu.colorLibSettings.newColorProfileFileName =
        option; C.ImportColorProfile(option); menu.refresh()
  end
  ...
  row[2].handlers.onClick = function()
    C.ImportColorMap(true); menu.refresh()
  end

  row[4]:setColSpan(2):createEditBox({ height = config.standardTextHeight, description = ReadText(1001, 12706) })
      :setText(menu.colorLibSettings.newColorProfileName,
        { fontsize = config.standardFontSize, x = Helper.standardTextOffsetx })
  ...
  row[4].handlers.onTextChanged = function(_, text)
    menu.colorLibSettings.newColorProfileName = text; menu.colorLibSettings.newColorProfileFileName = utf8.gsub(
      menu.colorLibSettings.newColorProfileName, "[^%w_%-%() ]", "_")
  end

  local row = buttontable:addRow(true, { fixed = true })
  row[4]:createButton({
    active = function()
      return (menu.colorLibSettings.newColorProfileFileName ~= "") and
          (menu.colorLibSettings.profilesByFilename[menu.colorLibSettings.newColorProfileFileName] ~= nil)
    end
  }):setText(
    ReadText(1001, 12707), { halign = "center" })
  row[4].handlers.onClick = function()
    C.ExportColorProfile(menu.colorLibSettings.newColorProfileFileName, menu.colorLibSettings.newColorProfileName); menu
        .refresh()
  end
  row[5]:createButton({
    active = function()
    ...
    ReadText(1001, 7909), { halign = "center" })
  row[5].handlers.onClick = function()
    C.ExportColorProfile(menu.colorLibSettings.newColorProfileFileName, menu.colorLibSettings.newColorProfileName); menu
        .refresh()
  end
  ...
  mappingtable.properties.maxVisibleHeight = (mappingtable.properties.maxVisibleHeight > 0) and
      math.min(mappingtable.properties.maxVisibleHeight, availablemappingheight) or availablemappingheight
  local availablecolorheight = buttontable.properties.y - colortable.properties.y - Helper.borderSize
  colortable.properties.maxVisibleHeight = (colortable.properties.maxVisibleHeight > 0) and
      math.min(colortable.properties.maxVisibleHeight, availablecolorheight) or availablecolorheight
...
function menu.buttonEditColor(colorid)
  ...
  menu.contextMenuData = {
    width = Helper.scaleX(400),
    x = mousepos.x + Helper.viewWidth / 2,
    y = mousepos.y +
        Helper.viewHeight / 2,
    colorid = colorid
  }
...
function menu.displayInputFeedback()
  ...
  local numcols = 5
  local optiontable = frame:addTable(numcols,
    { tabOrder = 1, x = menu.table.x, y = menu.table.y, width = menu.table.width })
  optiontable:setColWidth(1, menu.table.arrowColumnWidth, false)
  ...
  if n > 0 then
    ...
    for i = 0, n - 1 do
      ...
      if control then
        menu.inputFeedbackSettings.config[control][id] = {
          type = type,
          idname = idname,
  ...
  local row = optiontable:addRow(true, { fixed = true })
  row[2]:setColSpan(4):createEditBox({ height = config.standardTextHeight, defaultText = ReadText(1001, 3250) }):setText(
    menu.searchtext, { x = Helper.standardTextOffsetx }):setHotkey("INPUT_STATE_DETAILMONITOR_0", { displayIcon = true })
  ...
  row[2]:createButton({}):setText(ReadText(1001, 2647), { halign = "center" })
  row[2].handlers.onClick = function()
    C.ImportInputFeedbackConfig(true); menu.refresh()
  end

  local optionheight = optiontable:getVisibleHeight()
  local buttonheight = buttontable:getFullHeight()
  buttontable.properties.y = math.min(optiontable.properties.y + optionheight + Helper.borderSize,
    Helper.viewHeight - menu.frameOffsetY - buttonheight - menu.table.x)

  optiontable.properties.maxVisibleHeight = math.min(optiontable.properties.maxVisibleHeight,
    buttontable.properties.y - optiontable.properties.y - Helper.borderSize)
...
function menu.displayInputFeedbackRow(ftable, controltype, controlcode, configentry)
  ...
  if configentry.textoption ~= "" then
    ...
    row[3].handlers.onDropDownConfirmed = function(_, option)
      menu.dropdownSetInputFeedbackTextOption(option, controltype,
        controlcode)
    end
  else
  ...
  if configentry.voiceoption ~= "" then
    ...
    row[5].handlers.onDropDownConfirmed = function(_, option)
      menu.dropdownSetInputFeedbackVoiceOption(option,
  ...
  for j, modifier in ipairs(config.input.modifiers) do
    ...
    if #modifierkeys > 0 then
      for i, input in ipairs(modifierkeys) do
        ...
        row[5].handlers.onClick = function()
          return menu.buttonDeleteModifier(modifier.id, input[1], input[2],
            #modifierkeys == 1)
        end

        if i == 1 then
          local row = optiontable:addRow(nil, {})
          row[3]:setColSpan(3):createText(" ",
            { fontsize = 1, height = Helper.borderSize, cellBGColor = Color["row_separator"] })
        end
      end
      ...
      row[3]:setColSpan(3):createButton({ active = active, mouseOverText = (not active) and ReadText(1026, 2674) or "" })
          :setText(
            function()
              return menu.nameModifier(modifier.controller and ReadText(1001, 12747) or ReadText(1001, 12647),
                row.index)
            end, { halign = "center" })
      ...
      row[3]:setColSpan(3):createButton({}):setText(
        function()
          return menu.nameModifier(modifier.controller and ReadText(1001, 12747) or ReadText(1001, 12647),
            row.index)
        end, { halign = "center" })
  ...
  if not lastkey then
    ...
    if #conflicts == 0 then
      ...
      menu.contextMenuData = {
        ...
        conflicts =
            conflicts,
        removemodifier = { modifier, source, code }
      }
...
function menu.displaySavegameOptions(optionParameter)
  ...
  for i, savegame in ipairs(menu.savegames) do
    ...
    if savegame.filename == "quicksave" then
      savegame.displayedname = ((savegame.description ~= "") and savegame.description or savegame.location) ..
          " (" .. ReadText(1001, 400) .. ")"
      quicksavegame = i
      if menu.currentOption == "load" then
    ...
    elseif string.find(savegame.filename, "autosave_", 1, true) == 1 then
      savegame.displayedname = string.format("%s (%s)",
        (savegame.description ~= "") and savegame.description or savegame.location, ReadText(1001, 406))
  ...
  -- list
  local ftable = frame:addTable(5,
    { tabOrder = 1, x = menu.table.x, y = 0, width = menu.table.widthExtraWide / 2, maxVisibleHeight = menu.table.height })
  ftable:setColWidth(1, menu.table.arrowColumnWidth, false)
  ...
  local isonline = Helper.isOnlineGame()
  local showonlinesave = C.IsVentureExtensionSupported() and menu.onlinesave and
      ((menu.currentOption == "load") or isonline) and (menu.currentOption ~= "saveoffline")
  ...
  if isonline and (menu.currentOption == "save") and (not menu.showofflinesaves) then
    ...
    row[2]:createButton({ height = config.infoTextHeight }):setIcon(
      (menu.saveSort == "slot_inv") and "table_arrow_inv_up" or "table_arrow_inv_down",
      {
        scaling = false,
        width = arrowWidth,
        height = arrowWidth,
        x = (row[2]:getColSpanWidth() - arrowWidth) / 2,
        color = ((menu.saveSort == "slot") or (menu.saveSort == "slot_inv")) and
            Color["icon_normal"] or Color["icon_hidden"]
      })
    row[2].handlers.onClick = function()
      menu.saveSort = (menu.saveSort == "slot") and "slot_inv" or "slot"; menu.refresh()
    end
    nameSortButton = row[3]:createButton({ height = config.infoTextHeight }):setText(ReadText(1001, 2809)):setIcon(
      (menu.saveSort == "name_inv") and "table_arrow_inv_up" or "table_arrow_inv_down",
      {
        scaling = false,
        width = arrowWidth,
        height = arrowWidth,
        x = row[3]:getColSpanWidth() + Helper.scrollbarWidth -
            arrowWidth,
        color = ((menu.saveSort == "name") or (menu.saveSort == "name_inv")) and Color["icon_normal"] or
            Color["icon_hidden"]
      })
    row[3].handlers.onClick = function()
      menu.saveSort = (menu.saveSort == "name") and "name_inv" or "name"; menu.refresh()
    end
    row[4]:setColSpan(showonlinesave and 2 or 1):createButton({ height = config.infoTextHeight }):setText(ReadText(1001,
      2691)):setIcon((menu.saveSort == "date_inv") and "table_arrow_inv_up" or "table_arrow_inv_down",
      {
        scaling = false,
        width = arrowWidth,
        height = arrowWidth,
        x = row[4]:getColSpanWidth() - arrowWidth,
        color = ((menu.saveSort == "date") or (menu.saveSort == "date_inv")) and
            Color["icon_normal"] or Color["icon_hidden"]
      })
    row[4].handlers.onClick = function()
      menu.saveSort = (menu.saveSort == "date") and "date_inv" or "date"; menu.refresh()
    end
    ...
    if (menu.saveSort == "slot") or (menu.saveSort == "slot_inv") then
      ...
      for i = startIdx, endIdx, direction do
        ...
        if usedsavegamenames["save_" .. savegamestring] then
          ...
          row[2]:createText(i,
            ((menu.currentOption == "save") or (menu.currentOption == "saveoffline")) and config.standardTextProperties or
            config.disabledTextProperties)
          row[2].properties.halign = "right"
          row[3]:setColSpan(3):createText(ReadText(1001, 4812),
            {
              scaling = false,
              font = config.font,
              fontsize = Helper.scaleFont(config.font, config.standardFontSize),
              x =
                  Helper.scaleX(config.standardTextOffsetX),
              y = Helper.scaleY(2),
              color = (menu.currentOption == "load") and
                  Color["text_inactive"] or nil,
              minRowHeight = Helper.scaleY(config.standardTextHeight) + Helper.borderSize
            })
          maxRowHeight = math.max(maxRowHeight, row:getHeight())
        end
      end
  ...
  if (menu.currentOption == "load") and (#customsaves > 0) and IsCheatVersion() then
    ...
    elseif menu.saveSort == "date_inv" then
      table.sort(customsaves, function(a, b) return a.rawtime < b.rawtime end)
    elseif (menu.saveSort == "name_inv") then
      table.sort(customsaves,
        function(a, b)
          return string.lower(string.sub(a.filename, prefixlen + 1)) >
              string.lower(string.sub(b.filename, prefixlen + 1))
        end)
    else
      table.sort(customsaves,
        function(a, b)
  ...
  local titletable = frame:addTable(4,
    {
      ...
      x = menu.table.x,
      y = menu.table.y,
      width = menu.table.widthExtraWide,
      maxVisibleHeight = menu.table
          .height,
      skipTabChange = true
    })
  titletable:setColWidth(1, menu.table.arrowColumnWidth, false)
  titletable:setColWidth(2,
    ftable.properties.width - menu.table.arrowColumnWidth - Helper.scaleY(config.infoTextHeight) - 2 * Helper.borderSize,
    false)
  ...
  row[1]:setBackgroundColSpan(4):createButton({ height = config.headerTextHeight }):setIcon(config.backarrow,
    { x = config.backarrowOffsetX })
  row[1].handlers.onClick = function() return menu.onCloseElement("back") end
  row[2]:setColSpan(3):createText(options.name, config.headerTextProperties)

  local offsety = titletable.properties.y + titletable:getFullHeight() + Helper.borderSize
  ftable.properties.y = offsety
  ftable.properties.maxVisibleHeight = math.max(menu.table.height - offsety,
    maxRowHeight + Helper.borderSize + (customsavetitlerow and (customsavetitlerow:getHeight() + Helper.borderSize) or 0))
  ...
  local buttontable = frame:addTable(4,
    {
      tabOrder = 2,
      x = menu.table.x + ftable.properties.width + Helper.borderSize,
      y = offsety,
      width = ftable
          .properties.width - Helper.borderSize,
      maxVisibleHeight = menu.table.height - offsety,
      highlightMode = "off"
    })
  ...
  if (menu.currentOption == "save") or (menu.currentOption == "saveoffline") then
    menu.saveNameEditBox = row[3]:setColSpan(2):createEditBox({
      ...
      description =
          ReadText(1001, 8970),
      active = function()
        return (menu.selectedOption ~= nil) and (next(menu.selectedOption) ~= nil) and
            (not menu.selectedOption.isonline)
      end
    }):setText(menu.savegameName,
      { fontsize = config.standardFontSize, halign = "right" }):setHotkey("INPUT_STATE_DETAILMONITOR_Y",
    ...
    row[3]:setColSpan(2):createText(
      function()
        return ((menu.selectedOption ~= nil) and (next(menu.selectedOption) ~= nil)) and
            menu.selectedOption.displayedname or ""
      end, config.standardRightTextProperties)
  end
  local inforows = {}
  -- player name
  local row = buttontable:addRow(nil, {})
  ...
  row[3]:setColSpan(2):createText(
    function()
      return ((menu.selectedOption ~= nil) and (next(menu.selectedOption) ~= nil)) and
          menu.selectedOption.playername or ""
    end, config.standardRightTextProperties)
  -- money
  local row = buttontable:addRow(nil, {})
  table.insert(inforows, row)
  ...
  row[3]:setColSpan(2):createText(
    function()
      return ((menu.selectedOption ~= nil) and (next(menu.selectedOption) ~= nil) and menu.selectedOption.playername) and
          (ConvertMoneyString(menu.selectedOption.money, false, true, 0, true, false) .. " " .. ReadText(1001, 101)) or
          ""
    end,
    config.standardRightTextProperties)
  -- location
  ...
  row[3]:setColSpan(2):createText(
    function()
      return ((menu.selectedOption ~= nil) and (next(menu.selectedOption) ~= nil) and menu.selectedOption.playername) and
          menu.selectedOption.location or ""
    end, config.standardRightTextProperties)
  -- game time
  local row = buttontable:addRow(nil, {})
  table.insert(inforows, row)
  ...
  row[3]:setColSpan(2):createText(
    function()
      return ((menu.selectedOption ~= nil) and (next(menu.selectedOption) ~= nil) and menu.selectedOption.playername) and
          ConvertTimeString(menu.selectedOption.playtime,
            "%d" .. ReadText(1001, 104) .. " %H" .. ReadText(1001, 102) .. " %M" .. ReadText(1001, 103)) or ""
    end,
    config.standardRightTextProperties)
  -- version
  local row = buttontable:addRow(nil, {})
  ...
  row[3]:setColSpan(2):createText(
    function()
      return ((menu.selectedOption ~= nil) and (next(menu.selectedOption) ~= nil) and menu.selectedOption.gamestart) and
          menu.selectedOption.gamestart or ""
    end, config.standardRightTextProperties)
  -- errors
  local row = buttontable:addRow(nil, {})
  table.insert(inforows, row)
  row[1]:setColSpan(4):createText(menu.errorSavegameInfo(),
    { color = (menu.currentOption == "load") and Color["text_error"] or Color["text_warning"], wordwrap = true })
  -- buttons
  local row = buttontable:addRow(true, {})
  if menu.currentOption == "load" then
    row[1]:createButton({
      bgColor = function()
        return menu.isValidSaveSelected() and Color["button_background_default"] or
            Color["button_background_inactive"]
      end,
      highlightColor = function()
        return menu.isValidSaveSelected() and
            Color["button_highlight_default"] or Color["button_highlight_inactive"]
      end
    }):setText(ReadText(1001, 8966),
      {
        halign = "center",
        color = function()
          return menu.isValidSaveSelected() and Color["text_normal"] or
              Color["text_inactive"]
        end
      })
    row[1].handlers.onClick = function()
      if menu.isValidSaveSelected() then
        menu.loadGameCallback(
          menu.selectedOption.filename, false)
      end
    end
  else
    row[1]:createButton({
      active = function()
        return (menu.selectedOption ~= nil) and (next(menu.selectedOption) ~= nil) and
            (menu.selectedOption.titlerow == nil) and (menu.selectedOption.submenu == nil)
      end
    }):setText(
      function()
        local isempty = menu.selectedOption and menu.selectedOption.empty; local isonlinesave = menu.selectedOption and
            menu.selectedOption.isonline; return (Helper.isOnlineGame() and (not isonlinesave)) and
            (isempty and ReadText(1001, 11701) or ReadText(1001, 11702)) or
            (isempty and ReadText(1001, 8967) or ReadText(1001, 8968))
      end, { halign = "center" })
  ...
  row[4]:createButton({
    active = function()
      return (menu.selectedOption ~= nil) and (next(menu.selectedOption) ~= nil) and
          (menu.selectedOption.titlerow == nil) and (menu.selectedOption.submenu == nil) and
          (not menu.selectedOption.empty)
    end
  })
      :setText(ReadText(1001, 8931), { halign = "center" }):setHotkey("INPUT_STATE_DETAILMONITOR_X",
    { displayIcon = true })
...
function menu.isValidSaveSelected()
  return (menu.selectedOption ~= nil) and (next(menu.selectedOption) ~= nil) and menu.selectedOption.filename and
      ((not menu.selectedOption.error and not menu.selectedOption.invalidpatches and not menu.selectedOption.invalidversion and not menu.selectedOption.invalidgameid) or IsCheatVersion())
end
...
function menu.displayOnlineLogin()
  ...
  local secondMainColumnWidth = menu.table.infoColumnWidth - Helper.scrollbarWidth - config.standardTextHeight -
      Helper.borderSize
  ...
  ftable:setColWidth(3,
    0.8 * menu.table.widthWithExtraInfo - Helper.scaleX(config.standardTextHeight) - menu.table.arrowColumnWidth -
    4 * Helper.borderSize, false)
  ...
  ftable:setColWidth(10, 4 * secondMainColumnWidth / 9 - Helper.scaleX(config.standardTextHeight) - Helper.borderSize,
    false)
  ...
  -- sub title
  local row = ftable:addRow(false, {})
  row[2]:setColSpan(10):createText(
    hassession and string.format(ReadText(1001, 8993), menu.onlineData.username) or ReadText(1001, 8962),
    config.standardTextProperties)
  if hassession then
  ...
  row[7]:setColSpan(4):createEditBox({
    description = ReadText(1001, 7246),
    active = active,
    textHidden = function()
      return
          menu.onlineData.usernameHidden
    end
  }):setText(menu.onlineData.username, { fontsize = config.standardFontSize })
  if active then
  ...
  row[11]:createButton({ active = function() return not menu.onlineData.usernameEditBoxActive end }):setIcon(
    "menu_hidden")
  ...
  row[7]:setColSpan(5):createEditBox({
    textHidden = true,
    encrypted = true,
    description = ReadText(1001, 7247),
    active =
        active
  }):setText(active and menu.onlineData.password or "      ", { fontsize = config.standardFontSize })
  if active then
  ...
  if OnlineHasSession() then
    local isonline = Helper.isOnlineGame()
    row[2]:setColSpan(5):createButton({
      height = config.standardTextHeight,
      active = not isonline,
      mouseOverText =
          isonline and ReadText(1026, 2654) or ""
    }):setText(ReadText(1001, 7250),
      { fontsize = config.standardFontSize, halign = "center" })
    ...
    row[2]:setColSpan(5):createButton({
      ...
      active = function()
        return active and
            (menu.onlineData.username ~= "") and (menu.onlineData.password ~= "") and
            (__CORE_GAMEOPTIONS_PRIVACYPOLICY == true)
      end
    }):setText(ReadText(1001, 7249),
      { fontsize = config.standardFontSize, halign = "center" })
  ...
  elseif menu.onlineData.loginError then
    ...
    if showhelpbutton then
      row[10]:setColSpan(2):createButton({ height = config.standardTextHeight, active = C.CanOpenWebBrowser() }):setText(
        ReadText(1001, 8992) .. " \27[mm_externallink]", { fontsize = config.standardFontSize, halign = "center" })
  ...
  if (not OnlineHasSession()) or (menu.onlineData.hasRegisteredGame == false) then
    ...
    row[1]:setColSpan(11):createText(
      (menu.onlineData.hasRegisteredGame == false) and ReadText(1001, 8965) or ReadText(1001, 8963),
      config.subHeaderTextProperties)
    -- no account text / not registered
    local row = ftable:addRow(false, {})
    row[2]:setColSpan(10):createText((menu.onlineData.hasRegisteredGame == false) and ReadText(1001, 8907) or
      ReadText(1001, 7291))
  end
  ...
  if (not OnlineHasSession()) or (menu.onlineData.hasRegisteredGame == false) then
    ...
    -- register
    row[2]:setColSpan(5):createButton({ height = config.standardTextHeight, active = C.CanOpenWebBrowser() }):setText(
      ReadText(1001, 7290) .. " \27[mm_externallink]", { fontsize = config.standardFontSize, halign = "center" })
    row[2].handlers.onClick = menu.buttonOnlineRegister
  end
  -- privacy policy
  row[7]:setColSpan(5):createButton({ height = config.standardTextHeight, active = C.CanOpenWebBrowser() }):setText(
    ReadText(1001, 7292) .. " \27[mm_externallink]", { fontsize = config.standardFontSize, halign = "center" })
  ...
  if C.IsVentureExtensionSupported() then
    ...
    row[7]:setColSpan(5):createText(isinstalled and ReadText(1001, 2617) or ReadText(1001, 2618),
      config.standardTextProperties)
    -- version
    local row = ftable:addRow(false, {})
    row[2]:setColSpan(5):createText(ReadText(1001, 2655), config.standardTextProperties)
    row[7]:setColSpan(5):createText(
      isinstalled and ffi.string(C.GetExtensionVersion(ventureextensionid, false)) or "---",
      config.standardTextProperties)
    -- status
    local row = ftable:addRow(false, {})
...
function menu.displayControls(optionParameter)
  ...
  ftable:setDefaultComplexCellProperties("dropdown", "text",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize })
  ftable:setDefaultComplexCellProperties("dropdown", "text2",
    { x = config.standardTextOffsetX, fontsize = config.standardFontSize, halign = "right" })

  local numheadercols = 6
  local headertable = frame:addTable(numheadercols,
    {
      tabOrder = 2,
      x = menu.table.x,
      y = menu.table.y,
  ...
  row[1]:createButton({ height = config.headerTextHeight }):setIcon(config.backarrow, { x = config.backarrowOffsetX })
  row[1].handlers.onClick = function() return menu.onCloseElement("back") end
  row[2]:setColSpan(numheadercols - 1):createText(firstpart .. ReadText(1001, 120) .. " " .. secondpart,
    config.headerTextProperties)
  ...
  for _, entry in ipairs(config.input.filters) do
    ...
    if name then
      ...
      row[col]:createButton({
        bgColor = (menu.controlsFilter == entry.id) and Color["row_background_selected"] or
            Color["row_title_background"],
        mouseOverText = mouseovertext
      }):setText(name, { halign = "center" })
      row[col].handlers.onClick = function()
        menu.controlsFilter = entry.id; menu.refresh()
      end
  ...
  row[2]:setColSpan(numheadercols - 1):createEditBox({
    height = config.standardTextHeight,
    defaultText = ReadText(1001,
      3250)
  }):setText(menu.searchtext, { x = Helper.standardTextOffsetx }):setHotkey("INPUT_STATE_DETAILMONITOR_0",
    { displayIcon = true })
  row[2].handlers.onEditBoxDeactivated = menu.editboxControlsSearchUpdateText
        ...
        if menu.filterControl(control[1], control[2], menu.searchtext) then
          ...
          if (not control.display) or control.display() then
            if IsCheatVersion() or (not config.input.cheatControls[control[1]][control[2]]) then
              menu.displayControlRow(ftable, i, control[1], control[2], control[3], control[4],
                controls.mappable or control[7], control[5], first, control[6], controls.compassmenusupport,
                controls.mouseonly, control.mousewheelonly)
            end
          end
          first = false
        end
...
function menu.filterName(id)
  ...
  elseif id == "keyboard" then
    return ReadText(1001, 12667) ..
        " " ..
        ColorText["text_input_device_keyboard"] ..
        menu.getInputDeviceIcon("keyboard") .. ColorText["text_input_device_mouse"] .. menu.getInputDeviceIcon("mouse")
  else
    ...
    if menu.mappedjoysticks[joystickid] and next(menu.mappedjoysticks[joystickid]) then
      if menu.mappedjoysticks[joystickid].xinput then
        ...
        return
            ReadText(1001, 12668) ..
            " " ..
            ColorText["text_input_device_controller"] ..
            menu.getInputDeviceIcon("controller") ..
            ((joystickid > 1) and menu.getInputDeviceIcon("number_" .. joystickid) or ""),
            menu.mappedjoysticks[joystickid].name
      else
        -- directinput axis
        return
            ReadText(1001, 12669) ..
            " " ..
            ColorText["text_input_device_joystick"] ..
            menu.getInputDeviceIcon("joystick") ..
            ((joystickid > 1) and menu.getInputDeviceIcon("number_" .. joystickid) or ""),
            menu.mappedjoysticks[joystickid].name
      end
    end
...
function menu.displayJoysticks()
    ...
    if joystick.guid ~= "" then
      ...
      for source, joystickindex in ipairs(mappedjoysticks) do
        if (joystickindex <= 8) and (joystickindex == i) then
          icon = (joystick.xinput and (ColorText["text_input_device_controller"] .. menu.getInputDeviceIcon("controller")) or (ColorText["text_input_device_joystick"] .. menu.getInputDeviceIcon("joystick"))) ..
              ((source > 1) and menu.getInputDeviceIcon("number_" .. source) or "")
        end
      end
      table.insert(options,
        { id = joystick.guid, text = joystick.name, text2 = icon, icon = "", displayremoveoption = false })
    end
  end
  ...
  for i, joystick in ipairs(mappedjoysticks) do
    ...
    row[2]:createDropDown(options, { active = isselectable, startOption = currentOption }):setTextProperties()
        :setText2Properties({ halign = "right" })
...
function menu.displayInputProfiles(optionParameter)
  ...
  row[2]:setColSpan(3):createText(
    (menu.currentOption == "profile_load") and ReadText(1001, 12684) or ReadText(1001, 12685),
    config.headerTextProperties)
  ...
  local ftable = frame:addTable(4,
    { tabOrder = 1, x = menu.table.x, y = offsety, width = menu.table.widthExtraWide / 2, maxVisibleHeight = height })
  ftable:setColWidth(1, menu.table.arrowColumnWidth, false)
  ...
  for i = 1, 5 do
    ...
    if userprofiles[filename] then
      ...
      if menu.currentOption == "profile_load" then
        ...
        row[2]:createEditBox({ description = ReadText(1001, 4858) }):setText(profile.name,
          { fontsize = config.standardFontSize })
      ...
      if menu.currentOption == "profile_load" then
        ...
        row[3]:createButton({ height = config.standardTextHeight }):setIcon("menu_save")
        row[3].handlers.onClick = function()
          return menu.buttonInputProfileSave({
            filename = filename,
            id = 100 + i,
            name =
                ReadText(1023, 100 + i),
  ...
  local infotable = frame:addTable(1,
    { tabOrder = 0, x = offsetx, y = offsety, width = menu.table.widthExtraWide / 2, maxVisibleHeight = height })

  local row = infotable:addRow(false, { bgColor = Color["optionsmenu_cell_background"] })
  row[1]:createText(menu.inputProfileDescription,
    {
      scaling = false,
      width = menu.table.widthExtraWide / 2,
      height = height,
      wordwrap = true,
      fontsize = Helper
          .scaleFont(config.font, config.infoFontSize)
    })
...
function menu.displayUserQuestion(question, callback, negCallback, timer, waitforgfx, editboxHook, editboxNote, infotext)
  ...
  if editboxHook then
    ...
    menu.questionEditBox = row[3]:setColSpan(2):createEditBox({
      height = config.standardTextHeight,
      description =
          ReadText(1001, 8936)
    }):setText(menu.userQuestion.editboxText, { fontsize = config.standardFontSize })
...
function menu.displayOnlineSeason(option)
  ...
  -- title
  local row = ftable:addRow(not menu.firstlanguageselection, { fixed = true })
  row[1]:setBackgroundColSpan(2):createButton({ height = config.headerTextHeight }):setIcon(config.backarrow,
    { x = config.backarrowOffsetX })
  row[1].handlers.onClick = function() return menu.onCloseElement("back") end
  ...
  row[2]:setColSpan(2):createText(
    (completed and ColorText["text_positive"] or "") ..
    counter .. ". " .. ReadText(1001, 11301) .. (completed and " \27[widget_tick_01]" or ""), config
  ...
  row[2]:setColSpan(2):createText(
    (completed and ColorText["text_positive"] or "") ..
    counter .. ". " .. ReadText(1001, 11358) .. (completed and " \27[widget_tick_01]" or ""), config
    .standardTextProperties)
  row[2].properties.x = 3 * config.standardTextOffsetX
  counter = counter + 1
  ...
  row[2]:setColSpan(2):createText(
    (completed and ColorText["text_positive"] or "") ..
    counter .. ". " .. ReadText(1001, 11302) .. (completed and " \27[widget_tick_01]" or ""), config
    .standardTextProperties)
  row[2].properties.x = 3 * config.standardTextOffsetX
  counter = counter + 1
  ...
  row[2]:setColSpan(2):createText(
    (completed and ColorText["text_positive"] or "") ..
    counter .. ". " .. ReadText(1001, 11573) .. (completed and " \27[widget_tick_01]" or ""), config
    .standardTextProperties)
  row[2].properties.x = 3 * config.standardTextOffsetX
  counter = counter + 1
  ...
  row[2]:setColSpan(2):createText(
    (completed and ColorText["text_positive"] or "") ..
    counter .. ". " .. ReadText(1001, 11575) .. (completed and " \27[widget_tick_01]" or ""), config
    .standardTextProperties)
  row[2].properties.x = 3 * config.standardTextOffsetX
  counter = counter + 1
...
function menu.displayInit(text)
  ...
  local ftable = menu.optionsFrame:addTable(1,
    {
      tabOrder = 0,
      width = math.floor(Helper.viewWidth / 2),
      x = math.floor(Helper.viewWidth / 4),
      y = math.floor(
        Helper.viewHeight / 2)
    })
    ...
    elseif (menu.currentOption == "new") or (menu.currentOption == "multiplayer_server") or (menu.currentOption == "new_timelines") or (menu.currentOption == "tutorials") then
      if menu.playNewGameCutscene and menu.playNewGameCutscene.movie then
        if not menu.playNewGameCutscene.cutsceneid then
          ...
          menu.playNewGameCutscene.cutsceneid = StartCutscene(menu.playNewGameCutscene.cutscenedesc,
            GetRenderTargetTexture(menu.rendertarget))
      ...
      if menu.newGameCutsceneTimer and (menu.newGameCutsceneTimer + 1 < curtime) then
        ...
        if menu.selectedOption and (menu.selectedOption.cutscene ~= "") then
          menu.playNewGameCutscene = {
            id = menu.selectedOption.id,
            movie = menu.selectedOption.cutscene,
            movievoice =
                menu.selectedOption.cutscenevoice
          }
          menu.refresh()
        end
      end
...
function menu.newGameCallback(option, checked)
  ...
  if menu.isStartmenu and (option.intro ~= "") then
    ...
    menu.animationDelay = { getElapsedTime() + option.introlength + option.introfadeoutlength, option, false, option
        .introfadeoutlength, 1.0 }
    ...
    if option.customeditor then
      table.insert(menu.history, 1,
        { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = option.id })
      __CORE_GAMEOPTIONS_RESTOREINFO.returnhistory = menu.history
      Helper.closeMenuAndOpenNewMenu(menu, "CustomGameMenu",
        { 0, 0, option.id, menu.currentOption == "multiplayer_server", option.id == "custom_creative", menu.paused ~= nil })
      menu.cleanup()
    elseif option.mapeditor then
      ...
      local hasnotuploadeditems = false
      for ware, waredata in Helper.orderedPairs(playerinventory) do
        local isbraneitem, isoperationvolatile, isseasonvolatile, isventureuploadallowed = GetWareData(ware,
          "isbraneitem", "isoperationvolatile", "isseasonvolatile", "isventureuploadallowed")
        if isbraneitem then
          local serveramount = 0
          if onlineitems[ware] then
            serveramount = onlineitems[ware].amount
      ...
      if (not checked) and (not menu.isStartmenu) and Helper.isOnlineGame() and hasnotuploadeditems then
        table.insert(menu.history, 1,
          { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = option.id })
        menu.displayUserQuestion(ReadText(1001, 2603) .. " - " .. ReadText(1001, 7720),
          function() return menu.newGameCallback(option, true) end, nil, nil, nil, nil, nil, ReadText(1001, 11707))
      else
        if menu.currentOption == "multiplayer_server" then
          Helper.addDelayedOneTimeCallbackOnUpdate(function() C.NewMultiplayerGame(option.id) end, true,
            getElapsedTime() + 0.1)
        elseif option.tutorial then
          ...
          if value == 1 then
            Helper.addDelayedOneTimeCallbackOnUpdate(
              function()
                Helper.closeMenuAndOpenNewMenu(menu, "UserQuestionMenu", { 0, 0, "starttutorial", { option.id, 1 } }); menu
                    .cleanup()
            ...
            Helper.addDelayedOneTimeCallbackOnUpdate(
              function()
                C.SetUserData("tutorial_started_from", tostring(value)); NewGame(option.id)
              end, true, getElapsedTime() + 0.1)
          end
        else
  ...
  if uitable == menu.optionTable then
    ...
    elseif menu.currentOption == "extensions" then
      if option == "defaults" then
        table.insert(menu.history, 1,
          { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = option })
    ...
    elseif menu.currentOption == "profile_load" then
      if option and next(option) and (not option.empty) then
        table.insert(menu.history, 1,
          { optionParameter = menu.currentOption, topRow = GetTopRow(menu.optionTable), selectedOption = option.id })
        menu.displayUserQuestion(ReadText(1001, 4859) .. " - \"" .. option.name .. "\"",
          function() return menu.callbackInputProfileLoad(option) end)
      end
    elseif menu.currentOption == "language" then
      if option and next(option) then
